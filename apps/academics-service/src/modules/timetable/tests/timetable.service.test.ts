...
const ctx: RequestContext = {
  ...
  params: {},
  body: {},
  tenantContext: { tenantId: "tenant", source: "jwt-claims", resolvedAt: new Date() },
  authContext: {
    source: "jwt-claims",
...
test("persistent timetable use cases validate a ready offering", async () => {
  ...
  await seed("teaching_groups", "group", {
    campusId: "campus",
    academicYearId: "year",
    type: "SECTION",
  });
  await seed("curriculum_subjects", "curriculum-subject", { subjectCatalogueId: "mathematics" });
  await seed("subject_offerings", "offering", {
    campusId: "campus",
    academicYearId: "year",
    curriculumSubjectId: "curriculum-subject",
    teachingGroupId: "group",
    requiredPeriodsPerWeek: 1,
    effectiveFrom: new Date("2026-06-01"),
  });
  await seed("teaching_assignments_v2", "assignment", {
  ...
  await seed("employee_campus_assignments", "campus-access", {
    employeeId: "teacher",
    campusId: "campus",
    availableForTeaching: true,
    effectiveFrom: new Date("2026-06-01"),
  });
  await seed("teacher_subject_eligibility", "eligibility", {
    employeeId: "teacher",
  ...
  const slot = await createPeriodSlotRecord(
    {
...
test("draft writes reject section collisions on create and update", async () => {
  const store = new InMemoryPlanningStore();
  const seed = (
    collection: Parameters<typeof store.insert>[0],
    id: string,
    values: Record<string, unknown>,
  ) =>
    store.insert(collection, "tenant", {
  ...
  await seed("timetable_period_slots", "slot-1", {
    periodSetId: "period-set",
    sequence: 1,
    slotType: "TEACHING",
  });
  await seed("timetable_period_slots", "slot-2", {
    periodSetId: "period-set",
    sequence: 2,
    slotType: "TEACHING",
  });
  ...
  await seed("teaching_assignments_v2", "assignment-a", {
    subjectOfferingId: "offering-a",
    employeeId: "teacher-a",
  });
  await seed("teaching_assignments_v2", "assignment-b", {
    subjectOfferingId: "offering-b",
    employeeId: "teacher-b",
  });
  const first = await addTimetableEntry(
    {
      timetableVersionId: "draft",
      dayOfWeek: "MONDAY",
      periodSlotIds: ["slot-1"],
      subjectOfferingId: "offering-a",
      teachingAssignmentIds: ["assignment-a"],
      entryType: "REGULAR",
    },
    ctx,
    { store },
  );
  const second = await addTimetableEntry(
    {
      timetableVersionId: "draft",
      dayOfWeek: "MONDAY",
      periodSlotIds: ["slot-2"],
      subjectOfferingId: "offering-b",
      teachingAssignmentIds: ["assignment-b"],
      entryType: "REGULAR",
    },
    ctx,
    { store },
  ...
  assert.equal(
    (await store.get("timetable_entries", "tenant", String(first.id)))?.status,
    "ACTIVE",
  );
  assert.deepEqual(
    (await store.get("timetable_entries", "tenant", String(second.id)))?.periodSlotIds,
    ["slot-2"],
  );
});
...
test("saving the same timetable constraint updates one persistent record", async () => {
  ...
  const created = await createTimetableConstraint({ ...base, parameters: { maximum: 7 } }, ctx, {
    store,
  });
  const updated = await createTimetableConstraint({ ...base, parameters: { maximum: 6 } }, ctx, {
    store,
  });
  const records = await store.list("timetable_constraints", "tenant", {
    timetableVersionId: "version",
  });
...
test("editable revision preserves sections and isolates entry changes", async () => {
  ...
  await seed("timetable_versions", "published", {
    campusId: "campus",
    academicYearId: "year",
    academicUnitId: "unit",
    academicLevelId: "class_1",
    programId: "program",
    name: "Class 1",
  ...
  await seed("timetable_versions", "stale-draft", {
    campusId: "campus",
    academicYearId: "year",
    academicUnitId: "unit",
    academicLevelId: "class_1",
    programId: "program",
    name: "Class 1 old draft",
    scopeType: "ACADEMIC_LEVEL",
    periodSetId: "period-set",
    status: "DRAFT",
    versionNumber: 1,
  ...
  await seed("timetable_versions", "section-draft", {
    campusId: "campus",
    academicYearId: "year",
    academicUnitId: "unit",
    academicLevelId: "class_1",
  ...
  await seed("subject_offerings", "offering-a", {
    campusId: "campus",
    academicYearId: "year",
    sectionId: "section-a",
  });
  await seed("subject_offerings", "offering-b", {
    campusId: "campus",
    academicYearId: "year",
    sectionId: "section-b",
  });
  ...
  await seed("timetable_entries", "entry-a", {
    timetableVersionId: "published",
    dayOfWeek: "MONDAY",
    periodSlotIds: ["slot-1"],
    subjectOfferingId: "offering-a",
    sectionId: "section-a",
  ...
  await seed("timetable_entries", "entry-b", {
    timetableVersionId: "published",
    dayOfWeek: "MONDAY",
    periodSlotIds: ["slot-1"],
    subjectOfferingId: "offering-b",
  ...
  assert.equal(
    (await store.get("timetable_versions", "tenant", "stale-draft"))?.status,
    "ARCHIVED",
  );
  assert.equal((await store.get("timetable_versions", "tenant", "section-draft"))?.status, "DRAFT");
  const cloned = await store.list("timetable_entries", "tenant", {
  ...
  assert.equal(
    (await store.get("timetable_entries", "tenant", String(sectionA.id)))?.dayOfWeek,
    "TUESDAY",
  );
  assert.equal(
    (await store.get("timetable_entries", "tenant", String(sectionB.id)))?.status,
    "INACTIVE",
  );
...
test("section timetable is bound to an existing class hierarchy", async () => {
  const store = new InMemoryPlanningStore();
  await store.insert("academics_classes", "tenant", {
    _id: "class_10",
    id: "class_10",
    tenantId: "tenant",
    campusId: "campus",
  ...
  await store.insert("academics_sections", "tenant", {
    _id: "section_a",
    id: "section_a",
    tenantId: "tenant",
    campusId: "campus",
...
test("complete section workflow rejects stale validation and preserves other section publications", async () => {
  ...
  assert.equal(
    (await store.get("timetable_versions", "tenant", "published_section_b"))?.status,
    "PUBLISHED",
  );
  assert.equal(
    (await store.get("timetable_versions", "tenant", "stale_section_a_draft"))?.status,
    "ARCHIVED",
  );
});
...
test("automatic generation fills a direct section offering and reports success honestly", async () => {
  const store = new InMemoryPlanningStore();
  const seed = (
    collection: Parameters<InMemoryPlanningStore["insert"]>[0],
    id: string,
    values: Record<string, unknown>,
  ) =>
    store.insert(collection, "tenant", {
  ...
  await seed("timetable_versions", "abandoned_draft", {
    campusId: "campus",
    academicYearId: "year",
    academicUnitId: "school",
    scopeType: "ACADEMIC_LEVEL",
    academicLevelId: "other_class",
    periodSetId: periodSet.id,
    status: "DRAFT",
  });
  await seed("timetable_entries", "abandoned_entry", {
    timetableVersionId: "abandoned_draft",
    dayOfWeek: "MONDAY",
    periodSlotIds: [slot.id],
    subjectOfferingId: "other_offering",
    teachingAssignmentIds: ["assignment_maths"],
  });
  await seed("timetable_versions", "current_published", {
    campusId: "campus",
    academicYearId: "year",
    academicUnitId: "school",
    programId: "secondary",
  ...
  await seed("timetable_entries", "current_published_entry", {
    timetableVersionId: "current_published",
    dayOfWeek: "MONDAY",
    periodSlotIds: [slot.id],
    subjectOfferingId: "offering_maths",
    sectionId: "section_a",

...
import {
  campusTransferRepository,
  type CampusTransferRepository,
} from "./campus-transfers.repository";
...
async function issueUniqueRegistrationNumber(
  students: StudentRepository,
  issue: (input: NumberingContext) => Promise<string>,
  context: Omit<NumberingContext, "stream" | "idempotencyKey">,
  key: string,
  studentId: string,
) {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    if (!(await students.registrationNumberExists(context.tenantId, value, studentId)))
      return value;
  }
  throw new ConflictError("unable to reserve a unique student registration number");
}
async function issueUniqueRollNumber(
  students: StudentRepository,
  issue: (input: NumberingContext) => Promise<string>,
  context: Omit<NumberingContext, "stream" | "idempotencyKey"> & {
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    if (
      !(await students.rollNumberExists(
        context.tenantId,
...
export async function createCampusTransfer(
  input: unknown,
  ...
  if (!student || student.student.status !== "ACTIVE")
    throw new NotFoundError("active student was not found");
  ...
  if (
    ...
    targetClass.campusId !== payload.targetCampusId
  ) {
  ...
  if (payload.targetSectionId) {
    ...
    if (
      ...
      section.classId !== targetClass.id ||
      section.campusId !== payload.targetCampusId
    ) {
  ...
  const targetRegistrationNumber =
    registrationAction === "KEEP"
      ? student.student.registrationNumber
      : await issueUniqueRegistrationNumber(
          students,
          issue,
          numberingContext,
          `campus-transfer-registration:${id}`,
          student.student.id,
        );
  const targetRollNumber = payload.targetSectionId
    ? await issueUniqueRollNumber(
        students,
        issue,
        {
          ...numberingContext,
  ...
  try {
    const execution = await deps.orchestrator.start({
      transferId: id,
      tenantId,
    ...
    const processing = await repository.update(
      ...
      id,
      {
        ...
        history: [
          ...created.history,
          { status: "PROCESSING", at: now, actorId: userId, note: "Workflow started" },
        ],
      },
      ["DRAFT"],
    );
    ...
    await repository.update(
      ...
      id,
      {
        ...
        history: [
          ...created.history,
          { status: "FAILED", at: now, actorId: userId, note: "Workflow could not start" },
        ],
      },
      ["DRAFT"],
    );
...
export async function listCampusTransfers(
  studentId: string,
  context: RequestContext,
  deps: Dependencies = {},
) {
  ...
  return (
    await (
      await resolve(deps.repository, campusTransferRepository)
    ).listByStudent(tenant(context), studentId)
  ).map(view);
}
export async function listCampusTransferPage(
  filter: CampusTransferPageFilter,
  context: RequestContext,
  deps: Dependencies = {},
) {
  permission(context, campusTransferPermissions.read);
  const page = await (
    await resolve(deps.repository, campusTransferRepository)
  ).listPage(tenant(context), filter);
  return { ...page, items: page.items.map(view) };
}
export async function getCampusTransfer(
  id: string,
  context: RequestContext,
  deps: Dependencies = {},
) {
  ...
  const value = await (
    await resolve(deps.repository, campusTransferRepository)
  ).getById(tenant(context), id);
  if (!value) throw new NotFoundError("campus transfer was not found");
  return view(value);
}
export async function approveCampusTransfer(
  id: string,
  context: RequestContext,
  deps: Dependencies = {},
) {
  ...
  const tenantId = tenant(context),
    ...
    repository = await resolve(deps.repository, campusTransferRepository);
  if (!deps.orchestrator) throw new Error("campus transfer orchestrator is not configured");
  const current = await repository.getById(tenantId, id);
  if (!current) throw new NotFoundError("campus transfer was not found");
  if (current.status !== "UNDER_REVIEW")
    throw new ConflictError("only a transfer under review can be approved");
  const now = deps.now?.() ?? new Date();
  const processing = await repository.update(
    tenantId,
    id,
    {
      ...
      history: [
        ...current.history,
        { status: "PROCESSING", at: now, actorId: userId, note: "Finance review approved" },
      ],
    },
    ["UNDER_REVIEW"],
  );
  try {
    ...
    return view(
      await repository.update(
        tenantId,
        id,
        { executionArn: execution.executionArn, updatedAt: now },
        ["PROCESSING"],
      ),
    );
  } catch (error) {
    ...
    await repository.update(
      ...
      id,
      {
        ...
        history: [
          ...processing.history,
          { status: "UNDER_REVIEW", at: now, actorId: userId, note: "Workflow restart failed" },
        ],
      },
      ["PROCESSING"],
    );
...
export async function retryCampusTransfer(
  id: string,
  context: RequestContext,
  deps: Dependencies = {},
) {
  ...
  if (current.status !== "FAILED")
    throw new ConflictError("only a failed campus transfer can be retried");
  const now = deps.now?.() ?? new Date();
  const processing = await repository.update(
    tenantId,
    id,
    {
      ...
      history: [
        ...current.history,
        { status: "PROCESSING", at: now, actorId: userId, note: "Workflow retry requested" },
      ],
    },
    ["FAILED"],
  );
    ...
    return view(
      await repository.update(
        ...
        id,
        {
          ...
          history: [
            ...processing.history,
            { status: "FAILED", at: now, actorId: userId, note: "Workflow retry could not start" },
          ],
        },
        ["PROCESSING"],
      ),
...
export async function cancelCampusTransfer(
  id: string,
  reason: string,
  context: RequestContext,
  deps: Dependencies = {},
  ...
  const tenantId = tenant(context),
    userId = actor(context),
    repository = await resolve(deps.repository, campusTransferRepository),
    current = await repository.getById(tenantId, id);
  if (!current) throw new NotFoundError("campus transfer was not found");
  const now = deps.now?.() ?? new Date();
  return view(
    await repository.update(
      ...
      id,
      {
        ...
        history: [
          ...current.history,
          { status: "CANCELLED", at: now, actorId: userId, note: reason.trim() },
        ],
      },
      ["DRAFT", "UNDER_REVIEW", "FAILED"],
    ),
  );
}
export async function updateCampusTransferWorkflow(
  tenantId: string,
  id: string,
  status: CampusTransferStatus,
    ...
    id,
    {
      ...
      history: [
        ...current.history,
        {
          ...
          at: now,
          actorId: "SYSTEM",
          ...(input.failureReason ? { note: input.failureReason } : {}),
        },
      ],
...
export async function commitCampusTransfer(
  tenantId: string,
  id: string,
  ...
  if (
    record.registrationAction === "REGENERATE" &&
    (await students.registrationNumberExists(
      tenantId,
      record.targetRegistrationNumber,
      record.studentId,
    ))
  ) {
    const targetClass = await (
      await resolve(deps.classes, () => classRepository)
    ).getById(tenantId, record.target.classId);
    if (!targetClass) throw new NotFoundError("target class was not found");
    const replacement = await issueUniqueRegistrationNumber(
      students,
      deps.numberIssuer ?? issueConfiguredNumber,
      {
        tenantId,
      ...
      `campus-transfer-registration:${id}:recovery`,
      record.studentId,
    );
    const now = deps.now?.() ?? new Date();
      ...
      id,
      {
        ...
        history: [
          ...record.history,
          {
            ...
            at: now,
            actorId: "SYSTEM",
            note: "Registration number reallocated after uniqueness reconciliation",
          },
        ],
      },
      [record.status],
  ...
  if (current.enrollment.id === record.target.enrollmentId) {
    if (
      record.registrationAction === "REGENERATE" &&
      current.student.registrationNumber !== record.targetRegistrationNumber
    )
      await students.setRegistrationNumber(
        tenantId,
        record.studentId,
        record.targetRegistrationNumber,
      );
    return view(
      await updateCampusTransferWorkflow(tenantId, id, "COMPLETED", workflowUpdate, deps),
    );
  }
  ...
  await students.changeEnrollment(tenantId, record.studentId, {
    ...
    ...(record.registrationAction === "REGENERATE"
      ? { registrationNumber: record.targetRegistrationNumber }
      : {}),

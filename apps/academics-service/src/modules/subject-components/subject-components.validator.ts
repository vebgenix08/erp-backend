...
const types: SubjectComponentType[] = [
  "THEORY",
  "LECTURE",
  "PRACTICAL",
  "LAB",
...
const number = (value: unknown, field: string, minimum = 0) => {
  if (value === undefined || value === null || value === "") return undefined;
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed < minimum)
    throw new ValidationError([{ field, message: `${field} must be ${minimum} or greater` }]);
  return parsed;
};
export function validateSubjectComponent(value: unknown): SubjectComponentInput {
  const input = object(value),
    ...
    componentType = text(input.componentType)?.toUpperCase() as SubjectComponentType | undefined;
  if (!curriculumSubjectId)
    throw new ValidationError([
      { field: "curriculumSubjectId", message: "curriculumSubjectId is required" },
    ]);
  if (!componentType || !types.includes(componentType))
    throw new ValidationError([
      { field: "componentType", message: "valid componentType is required" },
    ]);
  ...
  return {
    curriculumSubjectId,
    componentType,
    ...(baselinePeriodsPerWeek !== undefined ? { baselinePeriodsPerWeek } : {}),
    ...(baselineContactHours !== undefined ? { baselineContactHours } : {}),
    ...(creditContribution !== undefined ? { creditContribution } : {}),
    ...(workloadMultiplier !== undefined ? { workloadMultiplier } : {}),
    ...(preferredSessionLength !== undefined ? { preferredSessionLength } : {}),
    requiresConsecutivePeriods: input.requiresConsecutivePeriods === true,
    ...(maximumPeriodsPerDay !== undefined ? { maximumPeriodsPerDay } : {}),
    ...(preferredRoomTypeId ? { preferredRoomTypeId } : {}),
    ...(maximumGroupSize !== undefined ? { maximumGroupSize } : {}),
  };
}

...
test("mixed delivery is represented by separate components", async () => {
  ...
  const theory = await repository.create("tenant", "admin", {
    curriculumSubjectId: "physics",
    componentType: "THEORY",
    baselinePeriodsPerWeek: 5,
  });
  const practical = await repository.create("tenant", "admin", {
    curriculumSubjectId: "physics",
    componentType: "PRACTICAL",
    baselinePeriodsPerWeek: 2,
    preferredSessionLength: 2,
    requiresConsecutivePeriods: true,
    workloadMultiplier: 1.5,
  });
  assert.equal(theory.preferredSessionLength, 1);
  assert.equal(practical.requiresConsecutivePeriods, true);
  await assert.rejects(
    () =>
      repository.create("tenant", "admin", {
        curriculumSubjectId: "physics",
        componentType: "THEORY",
      }),
    /already exists/,
...
test("year plan owns a bounded component plan and activates once", async () => {
  ...
  const plan = await repository.create("tenant", "admin", {
    campusId: "campus",
    academicYearId: "year",
    academicUnitId: "unit",
    curriculumId: "cbse",
    ...
    componentPlans: [
      {
        subjectComponentId: "theory",
        plannedPeriodsPerWeek: 5,
  ...
  assert.equal((await repository.activate("tenant", "admin", plan.id))?.status, "ACTIVE");
...
test("activating an all-section plan creates direct section offerings without section groups", async () => {
    ...
    context: RequestContext = {
      ...
      authContext: {
        ...
        user: {
          id: "admin",
          source: "jwt-claims",
          permissions: ["academics.subject-plan.activate"],
        },
      },
    };
  const component = await componentRepository.create("tenant", "admin", {
    curriculumSubjectId: "physics",
    componentType: "THEORY",
  });
  ...
  await activateSubjectPlan(plan.id, context, {
    repository,
    componentRepository,
    planningStore: store,
  });

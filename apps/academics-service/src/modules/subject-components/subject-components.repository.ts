...
import type {
  SubjectComponentFilter,
  SubjectComponentInput,
  SubjectComponentRecord,
} from "./subject-components.model";
const clone = (r: SubjectComponentRecord): SubjectComponentRecord => ({
  ...r,
  createdAt: new Date(r.createdAt),
  updatedAt: new Date(r.updatedAt),
  ...(r.deactivatedAt ? { deactivatedAt: new Date(r.deactivatedAt) } : {}),
});
export interface SubjectComponentRepository {
  ...
  create(
    tenantId: string,
    actorId: string,
    input: SubjectComponentInput,
  ): Promise<SubjectComponentRecord>;
  deactivate(
    tenantId: string,
    actorId: string,
    id: string,
    reason: string,
  ): Promise<SubjectComponentRecord | null>;
}
export class InMemorySubjectComponentRepository implements SubjectComponentRepository {
  ...
  async list(tenantId: string, filter: SubjectComponentFilter) {
    return [...this.records.values()]
      .filter(
        (r) =>
          r.tenantId === tenantId &&
          Object.entries(filter).every(([k, v]) => r[k as keyof SubjectComponentRecord] === v),
      )
      .map(clone);
  }
    ...
    if (
      [...this.records.values()].some(
        (r) =>
          ...
          r.componentType === input.componentType &&
          r.status === "ACTIVE",
      )
    )
      throw new ConflictError("active subject component already exists");
    const now = new Date(),
      id = `subject_component_${crypto.randomUUID()}`;
    const record: SubjectComponentRecord = {
      id,
      tenantId,
      ...input,
      workloadMultiplier: input.workloadMultiplier ?? 1,
      preferredSessionLength: input.preferredSessionLength ?? 1,
      requiresConsecutivePeriods: input.requiresConsecutivePeriods ?? false,
      status: "ACTIVE",
      createdAt: now,
      createdBy: actorId,
      updatedAt: now,
...
interface Document extends SubjectComponentRecord {
  _id: string;
}
class MongoSubjectComponentRepository implements SubjectComponentRepository {
  ...
  async deactivate(tenantId: string, actorId: string, id: string, reason: string) {
    ...
    if (!current) return null;
    const now = new Date();
    const next: SubjectComponentRecord = {
      ...current,
      status: "INACTIVE",
      deactivatedAt: now,
      deactivatedBy: actorId,
    ...
    if (!replaced) throw new ConflictError("subject component was changed by another request");
    return clone(next);
  }
}
...
export function subjectComponentRepository(runtime: MongoEnvLike = env()) {
  return (singleton ??= (async () => {
    if (
      ...
      return new InMemorySubjectComponentRepository();

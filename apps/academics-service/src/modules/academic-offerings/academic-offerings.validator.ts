...
const capacity = (value: unknown) => {
  if (typeof value !== "number" || !Number.isInteger(value) || value < 1)
    throw new BadRequestError("capacity must be a positive whole number");
  return value;
};
export function validateAcademicOfferingCreateInput(input: unknown): AcademicOfferingCreateInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("academic offering input is required");
  ...
  if (value.sectionId !== undefined && text(value.sectionId))
    result.sectionId = text(value.sectionId);
...
export function validateAcademicOfferingUpdateInput(input: unknown): AcademicOfferingUpdateInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("academic offering update input is required");
  const value = input as Record<string, unknown>;
  const result: AcademicOfferingUpdateInput = {};
  ...
  if (value.status !== undefined) {
    ...
    if (!["ACTIVE", "INACTIVE"].includes(status))
      throw new BadRequestError("academic offering status is invalid");
...
export function validateAcademicOfferingListFilter(input: unknown): AcademicOfferingListFilter {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("academic offering filter must be an object");
  const value = input as Record<string, unknown>;
  const result: AcademicOfferingListFilter = {};
  for (const field of [
    "campusId",
    "academicYearId",
    "curriculumId",

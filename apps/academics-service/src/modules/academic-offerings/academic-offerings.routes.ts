...
export function registerAcademicOfferingRoutes(
  router: ApiRouter,
  deps: AcademicOfferingServiceDeps = {},
) {
  router.route("GET", "/academic-offerings", async (context) =>
    jsonResponse(200, await listAcademicOfferings(context, context.query, deps)),
  );
  ...
  router.route("POST", "/academic-offerings", async (context) =>
    jsonResponse(201, await createAcademicOffering(context.body, context, deps)),
  );

...
import {
  academicOfferingRepository,
  type AcademicOfferingRepository,
} from "./academic-offerings.repository";
import {
  validateAcademicOfferingCreateInput,
  validateAcademicOfferingListFilter,
  validateAcademicOfferingUpdateInput,
} from "./academic-offerings.validator";
...
async function validateHierarchy(
  tenant: string,
  input: ReturnType<typeof validateAcademicOfferingCreateInput>,
  deps?: AcademicOfferingServiceDeps,
) {
  const repositories = await resolve(deps);
  if (deps?.settings) {
    if (!(await deps.settings.campusExists(tenant, input.campusId)))
      throw new NotFoundError("campus not found");
    if (!(await deps.settings.academicYearExists(tenant, input.academicYearId)))
  ...
  const [curriculum, program, academicClass, section] = await Promise.all([
    ...
    input.sectionId
      ? repositories.sections.getById(tenant, input.sectionId)
      : Promise.resolve(null),
  ]);
  if (!curriculum || curriculum.status !== "ACTIVE")
    throw new NotFoundError("active curriculum not found");
  if (!program || program.status !== "ACTIVE" || program.campusId !== input.campusId)
    throw new NotFoundError("active program not found for campus");
  if (
    !academicClass ||
    academicClass.status !== "ACTIVE" ||
    academicClass.campusId !== input.campusId ||
    academicClass.programId !== input.programId
  ) {
  ...
  if (
    ...
    (!section ||
      ...
      section.campusId !== input.campusId ||
...
export async function listAcademicOfferings(
  ...
  return (
    await (
      await resolve(deps)
    ).repository.list(tenantId(context), validateAcademicOfferingListFilter(filter))
  ).map(toAcademicOfferingView);
}
export async function getAcademicOffering(
  id: string,
  context: RequestContext,
  deps?: AcademicOfferingServiceDeps,
) {
...
export async function createAcademicOffering(
  ...
  const validated = validateAcademicOfferingCreateInput(input);
...
export async function updateAcademicOffering(
  ...
  const tenant = tenantId(context);
  const repositories = await resolve(deps);
  const record = await repositories.repository.update(
    tenant,
    id,
    validateAcademicOfferingUpdateInput(input),
  );
  return record ? toAcademicOfferingView(record) : null;
}
export async function deactivateAcademicOffering(
  id: string,
  context: RequestContext,
  deps?: AcademicOfferingServiceDeps,
) {

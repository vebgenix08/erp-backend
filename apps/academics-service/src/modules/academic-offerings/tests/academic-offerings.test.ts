...
const context: RequestContext = {
  ...
  authContext: {
    ...
    user: {
      id: "admin",
      permissions: Object.values(academicOfferingPermissions),
      source: "request",
    },
  },
};
test("academic offering enforces the full local hierarchy", async () => {
  ...
  const curriculum = await curricula.create("tenant_one", {
    name: "CBSE Curriculum",
    type: "CBSE",
  });
  const program = await programs.create("tenant_one", {
    campusId: "campus_main",
    academicUnitId: "unit_school",
    code: "PRI",
    name: "Primary School",
  });
  const academicClass = await classes.create("tenant_one", {
    campusId: "campus_main",
    programId: program.id,
    code: "C01",
    name: "Class 1",
  });
  const section = await sections.create("tenant_one", {
    campusId: "campus_main",
    programId: program.id,
  ...
  const offering = await createAcademicOffering(
    ...
    context,
    { repository, curricula, programs, classes, sections },
  );

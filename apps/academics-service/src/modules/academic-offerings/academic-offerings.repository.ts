...
import type {
  AcademicOfferingCreateInput,
  AcademicOfferingListFilter,
  AcademicOfferingRecord,
  AcademicOfferingUpdateInput,
} from "./academic-offerings.model";
...
export interface AcademicOfferingRepository {
  ...
  update(
    tenantId: string,
    id: string,
    input: AcademicOfferingUpdateInput,
  ): Promise<AcademicOfferingRecord | null>;
  deactivate(tenantId: string, id: string): Promise<AcademicOfferingRecord | null>;
}
interface Document extends AcademicOfferingRecord {
  _id: string;
  offeringKey: string;
}
const tenant = (value: string) => {
  const id = value.trim();
  if (!id) throw new BadRequestError("tenantId is required");
  return id;
};
const key = (
  input: Pick<
    AcademicOfferingRecord,
    "campusId" | "academicYearId" | "curriculumId" | "programId" | "classId" | "sectionId"
  >,
) =>
  [
    input.campusId,
...
const clone = (record: AcademicOfferingRecord): AcademicOfferingRecord => ({
  ...record,
...
const toDocument = (record: AcademicOfferingRecord): Document => ({
  ...clone(record),
  _id: record.id,
  offeringKey: key(record),
});
...
export class InMemoryAcademicOfferingRepository implements AcademicOfferingRepository {
    ...
    return [...this.bucket(tenantId).values()]
      .filter((item) =>
        Object.entries(filter).every(
          ([field, value]) => !value || item[field as keyof AcademicOfferingRecord] === value,
        ),
      )
      .map(clone);
  }
  async getById(tenantId: string, id: string) {
    const record = this.bucket(tenantId).get(id);
    return record ? clone(record) : null;
  }
  async create(tenantId: string, input: AcademicOfferingCreateInput) {
    const bucket = this.bucket(tenantId);
    if ([...bucket.values()].some((item) => key(item) === key(input as AcademicOfferingRecord)))
      throw new ConflictError("academic offering already exists");
    const timestamp = new Date();
    const record: AcademicOfferingRecord = {
      id: `offering_${crypto.randomUUID()}`,
  ...
  async update(tenantId: string, id: string, input: AcademicOfferingUpdateInput) {
    ...
    if ([...bucket.values()].some((item) => item.id !== id && key(item) === key(next)))
      throw new ConflictError("academic offering already exists");
    bucket.set(id, next);
    return clone(next);
  }
...
class MongoAcademicOfferingRepository implements AcademicOfferingRepository {
  ...
  async list(tenantId: string, filter: AcademicOfferingListFilter) {
    ...
    return (await this.collection.findMany(query))
      .map(fromDocument)
      .filter((item): item is AcademicOfferingRecord => Boolean(item));
  }
  async getById(tenantId: string, id: string) {
  ...
  async create(tenantId: string, input: AcademicOfferingCreateInput) {
    ...
    const offeringKey = key(input as AcademicOfferingRecord);
    if (await this.collection.findOne({ tenantId: normalizedTenant, offeringKey }))
      throw new ConflictError("academic offering already exists");
    const timestamp = new Date();
    const record: AcademicOfferingRecord = {
      id: `offering_${crypto.randomUUID()}`,
  ...
  async update(tenantId: string, id: string, input: AcademicOfferingUpdateInput) {
    ...
    const duplicate = await this.collection.findOne({
      tenantId: tenant(tenantId),
      offeringKey: key(next),
    });
    if (duplicate && duplicate.id !== id)
      throw new ConflictError("academic offering already exists");
    await this.collection.replaceOne({ tenantId: tenant(tenantId), _id: id }, toDocument(next));
    return next;
  }
  async deactivate(tenantId: string, id: string) {
    return this.update(tenantId, id, { status: "INACTIVE" });
...
export async function createAcademicOfferingRepository(
  env: MongoEnvLike = runtimeEnv(),
): Promise<AcademicOfferingRepository> {

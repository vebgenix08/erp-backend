...
function context(tenantId = "tenant_one"): RequestContext {
  return {
    ...
    authContext: {
      ...
      user: {
        id: "admin",
        permissions: ["academics.teaching-assignment.read", "academics.teaching-assignment.manage"],
        source: "jwt-claims",
      },
    },
  };
...
test("creates and lists a tenant-isolated subject-teacher assignment", async () => {
  ...
  assert.equal(
    (await listTeachingAssignments(context("tenant_two"), { campusId: "campus_1" }, { repository }))
      .length,
    0,
  );
});
test("allows only one active in-charge per section and supports deactivation", async () => {
  const repository = new InMemoryTeachingAssignmentRepository();
  const first = await createTeachingAssignment(
    { ...input, role: "SECTION_INCHARGE", subjectId: undefined },
    context(),
    { repository },
  );
  ...
  assert.equal(
    (await deactivateTeachingAssignment(first.id, context(), { repository })).status,
    "INACTIVE",
  );
});

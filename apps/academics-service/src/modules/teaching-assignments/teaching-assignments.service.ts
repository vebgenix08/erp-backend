...
} from "./teaching-assignments.model";
import {
  teachingAssignmentRepository,
  type TeachingAssignmentRepository,
} from "./teaching-assignments.repository";

export interface TeachingAssignmentDeps {
  repository?: TeachingAssignmentRepository | Promise<TeachingAssignmentRepository>;
}
...
const repo = async (deps?: TeachingAssignmentDeps) =>
  await (deps?.repository ?? teachingAssignmentRepository());
const view = (record: Awaited<ReturnType<TeachingAssignmentRepository["create"]>>) => ({
  ...record,
...
function validateInput(input: unknown): TeachingAssignmentInput {
  ...
  const role = required(value.role, "role") as TeachingAssignmentRole;
  if (!["SUBJECT_TEACHER", "SECTION_INCHARGE"].includes(role))
    throw new BadRequestError("role is invalid");
  const subjectId =
    typeof value.subjectId === "string" && value.subjectId.trim()
      ? value.subjectId.trim()
      : undefined;
  if (role === "SUBJECT_TEACHER" && !subjectId)
    throw new BadRequestError("subjectId is required for a subject teacher");
  return {
    campusId: required(value.campusId, "campusId"),
    academicYearId: required(value.academicYearId, "academicYearId"),
    employeeId: required(value.employeeId, "employeeId"),
    employeeName: required(value.employeeName, "employeeName"),
    role,
...
export async function listTeachingAssignments(
  ...
  const parsed: TeachingAssignmentFilter = { campusId: required(value.campusId, "campusId") };
  for (const field of [
    "academicYearId",
    "employeeId",
    "classId",
...
export async function createTeachingAssignment(
  ...
  return view(
    await (await repo(deps)).create(tenant(context), actor(context), validateInput(input)),
  );
}
export async function deactivateTeachingAssignment(
  ...
  if (!record) throw new NotFoundError("teaching assignment not found");
  return view(record);
}

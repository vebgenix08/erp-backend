...
interface Document extends TeachingAssignmentRecord {
  _id: string;
}
const clone = (record: TeachingAssignmentRecord): TeachingAssignmentRecord => ({
  ...record,
  createdAt: new Date(record.createdAt),
  updatedAt: new Date(record.updatedAt),
  ...(record.deactivatedAt ? { deactivatedAt: new Date(record.deactivatedAt) } : {}),
});
const key = (value: TeachingAssignmentInput) =>
  [
    value.campusId,
...
export class InMemoryTeachingAssignmentRepository implements TeachingAssignmentRepository {
  ...
  async create(tenantId: string, actorId: string, input: TeachingAssignmentInput) {
    const duplicate = [...this.records.values()].find(
      (item) => item.tenantId === tenantId && item.status === "ACTIVE" && key(item) === key(input),
    );
    ...
    const at = new Date();
    const record: TeachingAssignmentRecord = {
      id: `teaching_assignment_${crypto.randomUUID()}`,
      tenantId,
      ...input,
      status: "ACTIVE",
      createdBy: actorId,
...
class MongoTeachingAssignmentRepository implements TeachingAssignmentRepository {
  ...
  async list(tenantId: string, filter: TeachingAssignmentFilter) {
    const query: TenantFilter<Document> = {
      tenantId,
  ...
  async create(tenantId: string, actorId: string, input: TeachingAssignmentInput) {
    ...
    if (
      ...
      throw new ConflictError("section already has an active in-charge");
    const at = new Date();
    const record: TeachingAssignmentRecord = {
      id: `teaching_assignment_${crypto.randomUUID()}`,
      tenantId,
  ...
  async deactivate(tenantId: string, id: string) {
    ...
    const result = await this.collection.findOneAndUpdate(
      { _id: id, tenantId },
      { $set: { status: "INACTIVE", updatedAt: at, deactivatedAt: at } },
...
export function teachingAssignmentRepository(): Promise<TeachingAssignmentRepository> {
  singleton ??= (async () => {
    ...
    if (!env.MONGODB_URI && !env.MONGODB_URI_DEV && !env.MONGODB_URI_PROD && !env.MONGODB_URI_TEST)
      return new InMemoryTeachingAssignmentRepository();
    const connection = await getMongoConnection(env);
    const collection = connection.client
      .db(connection.dbName)
      .collection<Document>("teaching_assignments");
    ...
    return new MongoTeachingAssignmentRepository(createTenantMongoCollection(collection));
  })();
  return singleton;
}

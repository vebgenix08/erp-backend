...
export async function deactivateSubjectUseCase(id: string, context: RequestContext, deps?: SubjectServiceDeps): Promise<SubjectView | null> {
  requireSubjectPermission(context, subjectPermissions.deactivate as Permission);
  const repository = await resolveSubjectRepository(deps);
  return toSubjectView(await repository.deactivate(requireSubjectTenantId(context), id));
}

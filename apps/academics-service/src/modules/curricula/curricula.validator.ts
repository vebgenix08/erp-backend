...
export function validateCurriculumCreateInput(input: unknown): CurriculumCreateInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("curriculum input is required");
  const value = input as Record<string, unknown>;
  const name = text(value.name);
  if (!name) throw new BadRequestError("curriculum name is required");
  return {
    name,
    type: curriculumType(value.type),
    authorityName: text(value.authorityName) || undefined,
  };
}

export function validateCurriculumUpdateInput(input: unknown): CurriculumUpdateInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("curriculum update input is required");
  const value = input as Record<string, unknown>;
  ...
  if (value.authorityName !== undefined)
    update.authorityName = text(value.authorityName) || undefined;
...
export function validateCurriculumListFilter(input: unknown): CurriculumListFilter {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("curriculum filter must be an object");
  const value = input as Record<string, unknown>;

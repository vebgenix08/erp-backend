...
interface CurriculumDocument extends CurriculumRecord {
  _id: string;
}
...
const toDocument = (record: CurriculumRecord): CurriculumDocument => ({
  ...clone(record),
  _id: record.id,
});
...
export class InMemoryCurriculumRepository implements CurriculumRepository {
  ...
  async getById(tenantId: string, id: string) {
    return this.bucket(tenantId).get(id) ? clone(this.bucket(tenantId).get(id)!) : null;
  }
  async create(tenantId: string, input: CurriculumCreateInput) {
    const bucket = this.bucket(tenantId);
    if (
      [...bucket.values()].some((item) => normalizedName(item.name) === normalizedName(input.name))
    ) {
    ...
    const record: CurriculumRecord = {
      ...
      code: `CUR-${id.slice(-6).toUpperCase()}`,
      ...input,
      status: "ACTIVE",
      createdAt: timestamp,
      updatedAt: timestamp,
  ...
  async update(tenantId: string, id: string, input: CurriculumUpdateInput) {
    ...
    if (
      input.name &&
      [...bucket.values()].some(
        (item) => item.id !== id && normalizedName(item.name) === normalizedName(input.name!),
      )
    ) {
      throw new ConflictError("curriculum name must be unique");
    }
    ...
    const next = clone({
      ...
      authorityName:
        input.authorityName !== undefined ? input.authorityName : current.authorityName,
      status,
      updatedAt: new Date(),
...
class MongoCurriculumRepository implements CurriculumRepository {
  ...
  async list(tenantId: string, filter: CurriculumListFilter) {
    const query: TenantFilter<CurriculumDocument> = { tenantId: tenant(tenantId) };
    if (filter.status) query.status = filter.status;
    if (filter.type) query.type = filter.type;
    return (await this.collection.findMany(query))
      .map(fromDocument)
      .filter((item): item is CurriculumRecord => Boolean(item))
      .sort((a, b) => a.name.localeCompare(b.name));
  }
  async getById(tenantId: string, id: string) {
  ...
  async create(tenantId: string, input: CurriculumCreateInput) {
    ...
    await this.collection.insertOne({
      ...toDocument(record),
      normalizedName: normalizedName(record.name),
  ...
  async update(tenantId: string, id: string, input: CurriculumUpdateInput) {
    ...
    await this.collection.replaceOne({ tenantId: tenant(tenantId), _id: id }, {
      ...toDocument(next),
      normalizedName: normalizedName(next.name),
    } as CurriculumDocument);
    return next;
  }
  async deactivate(tenantId: string, id: string) {
...
export async function createCurriculumRepository(
  env: MongoEnvLike = runtimeEnv(),
): Promise<CurriculumRepository> {

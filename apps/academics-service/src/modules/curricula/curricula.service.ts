...
import {
  validateCurriculumCreateInput,
  validateCurriculumListFilter,
  validateCurriculumUpdateInput,
} from "./curricula.validator";

export interface CurriculumServiceDeps {
  repository?: CurriculumRepository | Promise<CurriculumRepository>;
}
...
const repository = async (deps?: CurriculumServiceDeps) =>
  await (deps?.repository ?? curriculumRepository);

export async function listCurricula(
  context: RequestContext,
  filter?: unknown,
  deps?: CurriculumServiceDeps,
) {
  permit(context, curriculumPermissions.read);
  return (
    await (await repository(deps)).list(tenantId(context), validateCurriculumListFilter(filter))
  ).map(toCurriculumView);
}
...
export async function createCurriculum(
  ...
  return toCurriculumView(
    await (await repository(deps)).create(tenantId(context), validateCurriculumCreateInput(input)),
  );
}
export async function updateCurriculum(
  ...
  ).update(tenantId(context), id, validateCurriculumUpdateInput(input));
  return record ? toCurriculumView(record) : null;
}
export async function deactivateCurriculum(
  id: string,
  context: RequestContext,
  deps?: CurriculumServiceDeps,
) {

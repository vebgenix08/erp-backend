...
export function registerCurriculumRoutes(router: ApiRouter, deps: CurriculumServiceDeps = {}) {
  router.route("GET", "/curricula", async (context) =>
    jsonResponse(200, await listCurricula(context, context.query, deps)),
  );
  ...
  router.route("POST", "/curricula", async (context) =>
    jsonResponse(201, await createCurriculum(context.body, context, deps)),
  );

...
import {
  calculateWorkload,
  InMemoryFacultyPlanningRepository,
} from "../faculty-planning.repository";
...
test("one employee can teach across campuses but primary assignments cannot overlap", () => {
  ...
  repository.assignCampus("tenant", "admin", {
    employeeId: "teacher",
    campusId: "a",
  ...
  repository.assignCampus("tenant", "admin", {
    employeeId: "teacher",
    campusId: "b",
    assignmentType: "SHARED_FACULTY",
    availableForTeaching: true,
    effectiveFrom,
    approvedBy: "admin",
    status: "ACTIVE",
  });
  repository.addEligibility("tenant", "admin", {
    employeeId: "teacher",
    subjectCatalogueId: "maths",
    effectiveFrom,
    status: "ACTIVE",
  });
  const input = {
...
test("section incharge is an academic responsibility, not a teaching assignment", () => {
  const repository = new InMemoryFacultyPlanningRepository();
  const responsibility = repository.addResponsibility("tenant", "admin", {
    employeeId: "teacher",
    academicYearId: "year",
    campusId: "campus",
...
test("workload is calculated across all campuses", () => {
  const summary = calculateWorkload(
    "teacher",
    [
      { employeeId: "teacher", campusId: "a", contactPeriods: 12, weightedUnits: 12 },
      { employeeId: "teacher", campusId: "b", contactPeriods: 5, weightedUnits: 7.5 },
    ],
    { maximumContactPeriodsPerWeek: 16, maximumWeightedUnitsPerWeek: 20 },
  );
  assert.equal(summary.contactPeriods, 17);
  assert.equal(summary.campusBreakdown.length, 2);
  assert.equal(summary.exceedsContactLimit, true);
});

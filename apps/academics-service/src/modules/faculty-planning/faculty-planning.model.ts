...
export interface EmployeeCampusAssignment extends FacultyAudit {
  employeeId: string;
  campusId: string;
  assignmentType: "PRIMARY" | "TEACHING" | "SHARED_FACULTY" | "VISITING" | "TEMPORARY";
...
export interface TeacherSubjectEligibility extends FacultyAudit {
  ...
  componentTypes?: SubjectComponentType[];
...
export interface OfferingTeacherAssignment extends FacultyAudit {
  ...
  effectiveFrom: Date;
  effectiveUntil?: Date;
  eligibilityStatus: "VERIFIED" | "OVERRIDDEN" | "NOT_REQUIRED";
  eligibilityOverrideReason?: string;
  eligibilityOverriddenBy?: string;
...
export interface TeacherWorkloadPolicy extends FacultyAudit {
  ...
  effectiveUntil?: Date;
  reason?: string;
  componentMultipliers: Partial<Record<SubjectComponentType, number>>;
  status: "ACTIVE" | "INACTIVE";
}

...
export class InMemoryPlanningStore implements PlanningStore {
  ...
  private collection(name: PlanningCollection) {
  ...
  async get(collection: PlanningCollection, tenantId: string, id: string) {
  ...
  async insert(collection: PlanningCollection, tenantId: string, document: PlanningDocument) {
    if (document.tenantId !== tenantId) throw new Error("tenant mismatch");
    this.collection(collection).set(document.id, structuredClone(document));
  ...
  async replace(
    ...
    const current = await this.get(collection, tenantId, id);
    if (!current || current.version !== expectedVersion) return null;
    this.collection(collection).set(id, structuredClone(document));
    return structuredClone(document);
  }
}
...
class MongoPlanningStore implements PlanningStore {
  private readonly adapters = new Map<
    PlanningCollection,
    Promise<CollectionAdapter<PlanningDocument>>
  >();
  ...
  private adapter(name: PlanningCollection) {
    ...
    if (!adapter) {
      adapter = (async () => {
        if (["identity_employees", "settings_campuses", "settings_academic_years"].includes(name)) {
          throw new Error(`${name} must be read through its owning service`);
        }
        const collection = await getCollection<PlanningDocument>(name, this.runtime);
  ...
  async list(
    ...
    ).findMany({ tenantId, ...filter }, { sort: { updatedAt: -1 } });
  }
  async get(collection: PlanningCollection, tenantId: string, id: string) {
    return await (await this.adapter(collection)).findOne({ tenantId, _id: id });
  }
  async insert(collection: PlanningCollection, tenantId: string, document: PlanningDocument) {
    if (document.tenantId !== tenantId) throw new Error("tenant mismatch");
    return await (await this.adapter(collection)).insertOne(document);
  }
  async replace(
    collection: PlanningCollection,
    tenantId: string,
    ...
    return await (
      await this.adapter(collection)
    ).replaceOne({ tenantId, _id: id, version: expectedVersion }, document);
  }
}
...
export function planningStore(runtime: MongoEnvLike = env()): PlanningStore {
  ...
  singleton =
    ...
    !runtime.MONGODB_URI_TEST
      ...
      : new MongoPlanningStore(runtime);
  return singleton;
}

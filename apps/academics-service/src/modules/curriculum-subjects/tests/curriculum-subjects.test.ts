...
const ctx = (tenantId = "tenant_one"): RequestContext => ({
  ...
  authContext: {
    ...
    user: {
      id: "admin",
      source: "jwt-claims",
      permissions: ["academics.curriculum-subject.create"],
    },
  },
});
test("curriculum subject requires an active tenant-owned catalogue record", async () => {
  const repository = new InMemoryCurriculumSubjectRepository(),
    catalogueRepository = new InMemorySubjectCatalogueRepository();
  const catalogue = await catalogueRepository.create("tenant_one", "admin", {
    name: "Mathematics",
  });
  const record = await createCurriculumSubject(
    {
      academicUnitId: "unit_school",
      curriculumId: "curriculum_cbse",
      programId: "program_school",
      academicLevelId: "level_10",

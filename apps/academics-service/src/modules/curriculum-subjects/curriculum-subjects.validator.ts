...
const categories: CurriculumSubjectCategory[] = [
  "CORE",
  "LANGUAGE",
  "ELECTIVE",
  "OPTIONAL",
...
const category = (value: unknown) => {
  ...
  if (!normalized || !categories.includes(normalized))
    throw new ValidationError([
      { field: "subjectCategory", message: "valid subjectCategory is required" },
    ]);
  return normalized;
};
const optionalCredits = (value: unknown) => {
  ...
  if (!Number.isFinite(credits) || credits < 0)
    throw new ValidationError([{ field: "credits", message: "credits must be zero or greater" }]);
  return credits;
};

export function validateCurriculumSubjectCreate(value: unknown): CurriculumSubjectCreateInput {
  const input = object(value);
  const localSubjectCode = text(input.localSubjectCode),
    gradingSchemeId = text(input.gradingSchemeId),
    examinationSchemeId = text(input.examinationSchemeId),
    credits = optionalCredits(input.credits);
  return {
    ...
    subjectCatalogueId: required(input, "subjectCatalogueId"),
    subjectCategory: category(input.subjectCategory),
    isMandatory: input.isMandatory === undefined ? true : input.isMandatory === true,
    ...(localSubjectCode ? { localSubjectCode } : {}),
...
export function validateCurriculumSubjectUpdate(value: unknown): CurriculumSubjectUpdateInput {
  ...
  return {
    expectedVersion,
    ...(input.subjectCategory !== undefined
      ? { subjectCategory: category(input.subjectCategory) }
      : {}),
...
export function validateCurriculumSubjectFilter(value: unknown): CurriculumSubjectFilter {
  const input = object(value),
    result: CurriculumSubjectFilter = {};
  for (const field of [
    "academicUnitId",
    "curriculumId",
    "programId",

...
import type {
  CurriculumSubjectCreateInput,
  CurriculumSubjectFilter,
  CurriculumSubjectRecord,
  CurriculumSubjectUpdateInput,
} from "./curriculum-subjects.model";
...
const naturalKey = (
  ...
  [
    record.academicUnitId,
    record.curriculumId,
    record.programId,
    record.academicLevelId,
    record.subjectCatalogueId,
  ].join("|");
...
export interface CurriculumSubjectRepository {
  ...
  create(
    tenantId: string,
    actorId: string,
    input: CurriculumSubjectCreateInput,
  ): Promise<CurriculumSubjectRecord>;
  update(
    tenantId: string,
    actorId: string,
    id: string,
    input: CurriculumSubjectUpdateInput,
  ): Promise<CurriculumSubjectRecord | null>;
  deactivate(
    tenantId: string,
    actorId: string,
    id: string,
...
export class InMemoryCurriculumSubjectRepository implements CurriculumSubjectRepository {
      ...
      .filter(
        (record) =>
          ...
          Object.entries(filter).every(
            ([key, value]) => record[key as keyof CurriculumSubjectRecord] === value,
          ),
      )
      .map(clone);
  }
    ...
    if (
      [...this.records.values()].some(
        (record) =>
          ...
          naturalKey(record) === naturalKey(input),
      )
    )
      throw new ConflictError("curriculum subject already exists");
    const now = new Date(),
      id = `curriculum_subject_${crypto.randomUUID()}`;
    const record: CurriculumSubjectRecord = {
      id,
      tenantId,
      ...input,
      status: "ACTIVE",
  ...
  async update(tenantId: string, actorId: string, id: string, input: CurriculumSubjectUpdateInput) {
    ...
    if (current.version !== input.expectedVersion)
      throw new ConflictError("curriculum subject was changed by another request");
    const { expectedVersion: _expectedVersion, ...patch } = input;
    const next: CurriculumSubjectRecord = {
      ...current,
      ...patch,
      updatedAt: new Date(),
      updatedBy: actorId,
...
interface Document extends CurriculumSubjectRecord {
  _id: string;
  naturalKey: string;
}
class MongoCurriculumSubjectRepository implements CurriculumSubjectRepository {
  ...
  private from(document: Document | null) {
    ...
    const { _id, naturalKey: _naturalKey, ...record } = document;
    return clone({ ...record, id: record.id || _id });
  }
  async list(tenantId: string, filter: CurriculumSubjectFilter) {
    return (await this.collection.findMany({ tenantId, ...filter })).map(
      (record) => this.from(record)!,
    );
  }
  async get(tenantId: string, id: string) {
  ...
  async create(tenantId: string, actorId: string, input: CurriculumSubjectCreateInput) {
    ...
    if (await this.collection.findOne({ tenantId, naturalKey: key, status: "ACTIVE" }))
      throw new ConflictError("curriculum subject already exists");
    const now = new Date(),
      id = `curriculum_subject_${crypto.randomUUID()}`;
    const record: CurriculumSubjectRecord = {
    ...
    await this.collection.insertOne({ ...record, _id: id, naturalKey: key });
    return clone(record);
  }
  async update(tenantId: string, actorId: string, id: string, input: CurriculumSubjectUpdateInput) {
    const current = await this.get(tenantId, id);
    ...
    const replaced = await this.collection.replaceOne(
      ...
      { ...next, _id: id, naturalKey: naturalKey(next) },
    );
    if (!replaced) throw new ConflictError("curriculum subject was changed by another request");
    return clone(next);
  }
  async deactivate(tenantId: string, actorId: string, id: string, reason: string) {
    const current = await this.get(tenantId, id);
...
export function curriculumSubjectRepository(runtime: MongoEnvLike = env()) {
  return (singleton ??= (async () => {
    if (
      ...
      return new InMemoryCurriculumSubjectRepository();
    const collection = await getCollection<Document>("curriculum_subjects", runtime);
    await collection.createIndex(
      { tenantId: 1, naturalKey: 1, status: 1 },

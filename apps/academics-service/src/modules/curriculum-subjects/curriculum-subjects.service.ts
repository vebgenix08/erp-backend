...
} from "./curriculum-subjects.repository";
import {
  validateCurriculumSubjectCreate,
  validateCurriculumSubjectFilter,
  validateCurriculumSubjectUpdate,
} from "./curriculum-subjects.validator";
...
export interface CurriculumSubjectDependencies {
  repository?: CurriculumSubjectRepository;
  catalogueRepository?: SubjectCatalogueRepository;
}
...
const permit = (ctx: RequestContext, permission: Permission) => {
...
export async function listCurriculumSubjects(
  ctx: RequestContext,
  filter: unknown,
  deps?: CurriculumSubjectDependencies,
) {
  permit(ctx, curriculumSubjectPermissions.read);
  const { repository } = await repositories(deps);
  return (await repository.list(tenant(ctx), validateCurriculumSubjectFilter(filter))).map(
    toCurriculumSubjectView,
  );
}
export async function createCurriculumSubject(
  ...
  permit(ctx, curriculumSubjectPermissions.create);
  const tenantId = tenant(ctx),
    validated = validateCurriculumSubjectCreate(input),
    { repository, catalogueRepository } = await repositories(deps);
  const catalogue = await catalogueRepository.get(tenantId, validated.subjectCatalogueId);
  if (!catalogue || catalogue.status !== "ACTIVE")
    throw new NotFoundError("active subject catalogue record was not found");
...
export async function updateCurriculumSubject(
  ...
  permit(ctx, curriculumSubjectPermissions.update);
  const { repository } = await repositories(deps);
  const record = await repository.update(
    tenant(ctx),
    actor(ctx),
    id,
    validateCurriculumSubjectUpdate(input),
  );
  if (!record) throw new NotFoundError("curriculum subject was not found");
  return toCurriculumSubjectView(record);
}
export async function deactivateCurriculumSubject(
  id: string,
  reason: string,
  ctx: RequestContext,
  deps?: CurriculumSubjectDependencies,
) {
  permit(ctx, curriculumSubjectPermissions.deactivate);

...
import {
  ...
  type SubjectCatalogueRepository,
} from "./subject-catalogue.repository";
import {
  validateSubjectCatalogueCreate,
  validateSubjectCatalogueFilter,
  validateSubjectCatalogueUpdate,
} from "./subject-catalogue.validator";

export interface SubjectCatalogueDependencies {
  repository?: SubjectCatalogueRepository | Promise<SubjectCatalogueRepository>;
}
const repo = async (deps?: SubjectCatalogueDependencies) =>
  await (deps?.repository ?? subjectCatalogueRepository);
...
const permit = (context: RequestContext, required: Permission) => {
  if (!context.authContext?.user?.permissions.includes(required))
    throw new ForbiddenError(`permission ${required} is required`);
};

export async function listSubjectCatalogue(
  context: RequestContext,
  filter?: unknown,
  deps?: SubjectCatalogueDependencies,
) {
  permit(context, subjectCataloguePermissions.read);
  return (
    await (await repo(deps)).list(tenant(context), validateSubjectCatalogueFilter(filter))
  ).map(toSubjectCatalogueView);
}
...
export async function createSubjectCatalogue(
  ...
  return toSubjectCatalogueView(
    await (
      await repo(deps)
    ).create(tenant(context), actor(context), validateSubjectCatalogueCreate(input)),
  );
}
export async function updateSubjectCatalogue(
  id: string,
  input: unknown,
  context: RequestContext,
  deps?: SubjectCatalogueDependencies,
  ...
  const record = await (
    await repo(deps)
  ).update(tenant(context), actor(context), id.trim(), validateSubjectCatalogueUpdate(input));
  if (!record) throw new NotFoundError("subject catalogue record was not found");
  return toSubjectCatalogueView(record);
}
export async function deactivateSubjectCatalogue(
  id: string,
  reason: string,
  context: RequestContext,
  deps?: SubjectCatalogueDependencies,
  ...
  const record = await (
    await repo(deps)
  ).deactivate(tenant(context), actor(context), id.trim(), reason.trim());
  if (!record) throw new NotFoundError("subject catalogue record was not found");
  return toSubjectCatalogueView(record);
}

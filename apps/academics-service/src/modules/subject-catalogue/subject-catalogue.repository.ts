...
import type {
  ...
  SubjectCatalogueRecord,
  SubjectCatalogueUpdateInput,
} from "./subject-catalogue.model";

interface Document extends SubjectCatalogueRecord {
  _id: string;
  normalizedName: string;
}
const clone = (value: SubjectCatalogueRecord): SubjectCatalogueRecord => ({
  ...value,
  createdAt: new Date(value.createdAt),
  updatedAt: new Date(value.updatedAt),
  ...(value.deactivatedAt ? { deactivatedAt: new Date(value.deactivatedAt) } : {}),
});
...
export interface SubjectCatalogueRepository {
  ...
  create(
    tenantId: string,
    actorId: string,
    input: SubjectCatalogueCreateInput,
  ): Promise<SubjectCatalogueRecord>;
  update(
    tenantId: string,
    actorId: string,
    id: string,
    input: SubjectCatalogueUpdateInput,
  ): Promise<SubjectCatalogueRecord | null>;
  deactivate(
    tenantId: string,
    actorId: string,
    id: string,
...
function matches(record: SubjectCatalogueRecord, filter: SubjectCatalogueFilter) {
  ...
  return (
    !search ||
    [record.name, record.code, record.shortName].some((item) =>
...
export class InMemorySubjectCatalogueRepository implements SubjectCatalogueRepository {
  ...
  async list(tenantId: string, filter: SubjectCatalogueFilter) {
  ...
  async create(tenantId: string, actorId: string, input: SubjectCatalogueCreateInput) {
    if (
      [...this.records.values()].some(
        (item) => item.tenantId === tenantId && normalized(item.name) === normalized(input.name),
      )
    )
      throw new ConflictError("subject catalogue name already exists");
    const id = `subject_catalogue_${crypto.randomUUID()}`,
      now = new Date();
    const record: SubjectCatalogueRecord = {
      id,
      tenantId,
      code: `SUB-${id.slice(-6).toUpperCase()}`,
      ...input,
      status: "ACTIVE",
      createdAt: now,
      createdBy: actorId,
  ...
  async update(tenantId: string, actorId: string, id: string, input: SubjectCatalogueUpdateInput) {
    ...
    if (current.version !== input.expectedVersion)
      throw new ConflictError("subject catalogue record was changed by another request");
    if (
      input.name &&
      [...this.records.values()].some(
        (item) =>
    ...
    const next: SubjectCatalogueRecord = {
      ...current,
      ...patch,
      id,
      tenantId,
...
class MongoSubjectCatalogueRepository implements SubjectCatalogueRepository {
  ...
  private from(document: Document | null) {
    ...
    const { _id, normalizedName: _normalizedName, ...record } = document;
    return clone({ ...record, id: record.id || _id });
  }
  async list(tenantId: string, filter: SubjectCatalogueFilter) {
    return (await this.collection.findMany({ tenantId }))
      .map((item) => this.from(item)!)
  ...
  async create(tenantId: string, actorId: string, input: SubjectCatalogueCreateInput) {
    if (await this.collection.findOne({ tenantId, normalizedName: normalized(input.name) }))
      throw new ConflictError("subject catalogue name already exists");
    const id = `subject_catalogue_${crypto.randomUUID()}`,
      now = new Date();
    const record: SubjectCatalogueRecord = {
    ...
    await this.collection.insertOne({
      ...record,
      _id: id,
      normalizedName: normalized(record.name),
    });
    return clone(record);
  }
  async update(tenantId: string, actorId: string, id: string, input: SubjectCatalogueUpdateInput) {
    ...
    const replaced = await this.collection.replaceOne(
      ...
      { ...next, _id: id, normalizedName: normalized(next.name) },
    );
    if (!replaced)
      throw new ConflictError("subject catalogue record was changed by another request");
    return clone(next);
...
export async function createSubjectCatalogueRepository(
  runtime: MongoEnvLike = env(),
): Promise<SubjectCatalogueRepository> {
  if (
    !runtime.MONGODB_URI &&
    ...
    return new InMemorySubjectCatalogueRepository();

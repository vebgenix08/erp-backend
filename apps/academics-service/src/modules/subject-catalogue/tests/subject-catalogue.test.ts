...
} from "../subject-catalogue.service";
...
test("subject catalogue is reusable and tenant isolated", async () => {
  ...
  const mathematics = await createSubjectCatalogue(
    { name: "Mathematics", shortName: "Maths", subjectDomain: "Science" },
    context(),
    { repository },
  );
  await createSubjectCatalogue(
    { name: "Kannada", subjectDomain: "Language" },
    context("tenant_two"),
    { repository },
  );
...
test("subject catalogue uses optimistic versioning and recorded deactivation", async () => {
  ...
  const updated = await updateSubjectCatalogue(
    created.id,
    { expectedVersion: 1, shortName: "Sci" },
    context(),
    { repository },
  );
  ...
  await assert.rejects(
    () =>
      updateSubjectCatalogue(
        created.id,
        { expectedVersion: 1, name: "General Science" },
        context(),
        { repository },
      ),
    /changed by another request/,
  );
  const inactive = await deactivateSubjectCatalogue(
    created.id,
    "Replaced by separate science disciplines",
    context(),
    { repository },
  );

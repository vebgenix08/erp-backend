...
test("legacy section group creation remains idempotent during migration", () => {
  ...
  const first = repository.ensureSectionGroup("tenant", "admin", base),
    second = repository.ensureSectionGroup("tenant", "admin", base);
  assert.equal(first.id, second.id);
});
test("language choice creates membership without changing home section", () => {
  const repository = new InMemoryLearningDeliveryRepository();
  const choiceGroup = repository.createChoiceGroup("tenant", "admin", {
    campusId: "campus",
    academicYearId: "year",
    academicUnitId: "school",
    curriculumId: "cbse",
  ...
  assert.throws(
    () =>
      repository.createStudentChoice("tenant", "admin", {
        ...input,
        curriculumSubjectId: "sanskrit",
      }),
    /already has/,
  );
});
test("offering targets a section directly and parallel blocks require multiple offerings", () => {
  const repository = new InMemoryLearningDeliveryRepository();
  const offering = repository.createOffering("tenant", "admin", {
    campusId: "campus",
    academicYearId: "year",
    subjectPlanId: "plan",
    curriculumSubjectId: "kannada",
  ...
  assert.throws(
    () =>
      repository.createParallelBlock("tenant", "admin", {
        campusId: "campus",
        academicYearId: "year",
        academicUnitId: "school",
        programId: "school",
        ...
        requiredOfferingIds: [offering.id],
        mustRunSimultaneously: true,
        status: "ACTIVE",
      }),
    /at least two/,
  );

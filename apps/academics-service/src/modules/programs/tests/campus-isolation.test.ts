...
test("program listing isolates campuses within the same tenant", async () => {
  ...
  await createProgramUseCase(
    { campusId: "campus_school", academicUnitId: "unit_school", name: "Primary School" },
    context,
    { repository },
  );
  await createProgramUseCase(
    { campusId: "campus_college", academicUnitId: "unit_degree", name: "Bachelor of Commerce" },
    context,
    { repository },
  );

  const schoolPrograms = await listProgramsUseCase(
    context,
    { repository },
    { campusId: "campus_school" },
  );
  const collegePrograms = await listProgramsUseCase(
    context,
    { repository },
  ...
  assert.deepEqual(
    schoolPrograms.map((item) => item.name),
    ["Primary School"],
  );
  assert.deepEqual(
    collegePrograms.map((item) => item.name),

...
const collisionResolutionIds = new Set(
  (process.env.DEACTIVATE_COLLISION_ENTRY_IDS || "")
    .split(",")
...
try {
  const drafts = await versions
    .find({ status: "DRAFT" })
    .project({ _id: 1, id: 1, tenantId: 1 })
    .toArray();
  report.drafts = drafts.length;
  for (const draft of drafts) {
    const timetableVersionId = String(draft.id || draft._id);
    const tenantId = String(draft.tenantId);
    const activeEntries = await entries
      .find({ tenantId, timetableVersionId, status: "ACTIVE" })
      .toArray();
    report.entries += activeEntries.length;
    const occupied = new Map();
    for (const entry of activeEntries) {
      const target = entry.sectionId
        ? `SECTION:${entry.sectionId}`
        : entry.subjectBatchId
          ? `BATCH:${entry.subjectBatchId}`
      ...
      for (const periodSlotId of Array.isArray(entry.periodSlotIds) ? entry.periodSlotIds : []) {
        ...
        const current = {
          ...
          subjectOfferingId: String(entry.subjectOfferingId),
        };
        if (previous)
      ...
      const current = await assignments
        .find({ tenantId, subjectOfferingId: entry.subjectOfferingId, status: "ACTIVE" })
        .sort({ assignmentRole: 1, createdAt: 1, id: 1 })
        .project({ _id: 1, id: 1 })
        .toArray();
      ...
      const storedIds = Array.isArray(entry.teachingAssignmentIds)
        ? entry.teachingAssignmentIds.map(String).sort()
        : [];
      ...
      if (apply)
        await entries.updateOne(
          ...
          {
            $set: {
              teachingAssignmentIds: currentIds,
              updatedAt: new Date(),
              updatedBy: "system:draft-timetable-reconciliation",
            },
            $inc: { version: 1 },
          },
        );
  ...
  for (const collision of report.collisions) {
    for (const entry of collision.entries) {
      ...
      const curriculum = offering
        ? await curriculumSubjects.findOne({
            tenantId: collision.tenantId,
            id: offering.curriculumSubjectId,
          })
        : null;
      const subject = curriculum
        ? await subjectCatalogue.findOne({
            tenantId: collision.tenantId,
            id: curriculum.subjectCatalogueId,
          })
        : null;
  ...
  if (apply && collisionResolutionIds.size) {
    const participants = new Map(
      report.collisions.flatMap((collision) =>
        collision.entries.map((entry) => [entry.entryId, { collision, entry }]),
      ),
    );
    for (const entryId of collisionResolutionIds) {
      const participant = participants.get(entryId);
      if (!participant)
        throw new Error(`${entryId} is not part of a detected draft timetable collision`);
      const result = await entries.updateOne(
        {
          tenantId: participant.collision.tenantId,
          id: entryId,
          timetableVersionId: participant.collision.timetableVersionId,
          status: "ACTIVE",
        },
        {
          $set: {
            ...
            deactivatedBy: "system:draft-timetable-reconciliation",
            deactivationReason:
              "Resolved duplicate section period during draft timetable reconciliation",
            updatedAt: new Date(),
            updatedBy: "system:draft-timetable-reconciliation",
          },
          $inc: { version: 1 },
    ...
    report.collisions = report.collisions.filter(
      (collision) => !collision.entries.some((entry) => collisionResolutionIds.has(entry.entryId)),
    );
  }
} finally {

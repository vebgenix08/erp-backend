...
export async function listPermissions(
  ...
  return (await repository.list(resolveTenantId(context))).map((permission) =>
    toPermissionView(permission),
  );
}
...
export async function createPermission(
  ...
  return toPermissionView(
    await repository.create(resolveTenantId(context), validatePermissionCreateInput(input)),
  );
}
...
export async function updatePermission(
  ...
  const repository = await resolveRepository(deps);
  return toPermissionView(
    await repository.update(resolveTenantId(context), id, validatePermissionUpdateInput(input)),
  );
}

...
async function audit(
  ...
  repository: IdentityAuditRepository | Promise<IdentityAuditRepository> | undefined,
  action: string,
  entityType: string,
  entityId: string,
  ...
  await (
    await (repository ?? identityAuditRepository())
  ).append({
...
export async function getAccessSnapshot(
  context: RequestContext,
  deps: { repository?: AccessRepository } = {},
) {
  requirePermission(context.authContext, accessPermissions.read);
  const repository = deps.repository ?? accessRepository;
  const tenantId = tenant(context);
  return {
    permissions: PERMISSION_CATALOG,
    assignments: [],
    rolePermissions: await repository.listRolePermissions(tenantId),
  };
}
export async function getAssignmentPage(
  filter: import("./access.model").AssignmentPageFilter,
  context: RequestContext,
  deps: { repository?: AccessRepository } = {},
) {
  requirePermission(context.authContext, accessPermissions.read);
  return (deps.repository ?? accessRepository).listAssignmentPage(tenant(context), filter);
}
export async function assignUserRole(
  ...
  deps: {
    ...
    auditRepository?: IdentityAuditRepository | Promise<IdentityAuditRepository>;
  } = {},
) {
  requirePermission(context.authContext, accessPermissions.assignRole);
  ...
  const scope = validateScope(value.scope),
    result = await (deps.repository ?? accessRepository).assignRole(
      tenant(context),
      userId,
      roleId,
      scope,
    );
  await audit(context, deps.auditRepository, "ROLE_ASSIGNED", "USER_ROLE_ASSIGNMENT", result.id, {
    userId,
    roleId,
    scope,
  });
...
export async function revokeUserRole(
  ...
  requirePermission(context.authContext, accessPermissions.assignRole);
  const result = await (deps.repository ?? accessRepository).revokeAssignment(tenant(context), id);
  if (result)
    await audit(
      context,
      ...
      result.id,
      { userId: result.userId, roleId: result.roleId, scope: result.scope },
    );
  return result;
}
export async function saveRolePermissions(
  ...
  requirePermission(context.authContext, accessPermissions.assignPermission);
  const value = input as Record<string, unknown>;
  const roleId = String(value?.roleId ?? "").trim();
  if (!roleId) throw new Error("roleId is required");
  const permissions = validatePermissions(value.permissions),
    result = await (deps.repository ?? accessRepository).setRolePermissions(
      tenant(context),
      roleId,
      permissions,
    );
  await audit(context, deps.auditRepository, "ROLE_PERMISSIONS_REPLACED", "ROLE", roleId, {
    permissions,
  });
  return result;
}

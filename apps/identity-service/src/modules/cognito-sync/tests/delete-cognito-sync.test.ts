...
import {
  createCognitoSyncUseCase,
  deleteCognitoSyncUseCase,
  listCognitoSyncUseCase,
} from "../use-cases";
...
test("delete cognito sync removes the record", async () => {
  ...
  const deleted = await deleteCognitoSyncUseCase(created.id, createCognitoSyncContext(), {
    repository,
  });

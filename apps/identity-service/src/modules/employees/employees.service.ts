...
import {
  employeeInviteAttemptRepository,
  type EmployeeInviteAttemptRepository,
} from "./employee-invite-attempts.repository";
import {
  employeeDeliveryEventRepository,
  type EmployeeDeliveryEventRepository,
} from "./employee-delivery-events.repository";
...
export interface EmployeeServiceDeps {
  ...
  deliveryEventRepository?:
    | EmployeeDeliveryEventRepository
    | Promise<EmployeeDeliveryEventRepository>;
}
...
async function reconcileErpIdentity(
  ...
  const reconciledStatus = identity.status === "CONFIRMED" ? "ACTIVE" : "INVITED";
  if (!user) {
  ...
  } else if (!user.authUserId || user.status !== reconciledStatus) {
...
async function completeInviteProvisioning(
  ...
  const updated = await (deps.repository ?? employeeRepository).update(
    employee.tenantId,
    employee.id,
    {
      userId: user.id,
...
export async function listEmployees(
  ...
  return (
    await (deps.repository ?? employeeRepository).list(
      tenant(context),
      validateEmployeeFilter(filter),
    )
  ).map(toEmployeeView);
}
export async function listEmployeePage(
  ...
  requirePermission(context.authContext, employeePermissions.read);
  const page = await (deps.repository ?? employeeRepository).listPage(
    tenant(context),
    validateEmployeeFilter(filter),
  );
...
export async function resolveEmployeeByPrincipal(
  tenantId: string,
  principalId: string,
  deps: EmployeeServiceDeps = {},
) {
  ...
  if (!normalizedTenantId || !normalizedPrincipalId) return null;

  const employees = deps.repository ?? employeeRepository;
  const direct = await employees.findByPrincipal(normalizedTenantId, normalizedPrincipalId);
  if (direct) return direct;

  const users = await (deps.userRepository ?? userRepository());
  const user = await users.getByAuthUserId(normalizedTenantId, normalizedPrincipalId);
  return user ? employees.findByPrincipal(normalizedTenantId, user.id) : null;
}

export async function listEmployeeInviteAttempts(
  employeeId: string,
  context: RequestContext,
  deps: EmployeeServiceDeps = {},
) {
  ...
  return (
    await (deps.inviteAttemptRepository ?? employeeInviteAttemptRepository).list(
      tenantId,
      employeeId,
    )
  ).map((value) => ({
...
export async function listEmployeeInviteDeliveryEvents(
  employeeId: string,
  context: RequestContext,
  deps: EmployeeServiceDeps = {},
) {
  requirePermission(context.authContext, employeePermissions.read);
  const tenantId = tenant(context),
    employee = await (deps.repository ?? employeeRepository).get(tenantId, employeeId);
  if (!employee) throw new NotFoundError("employee not found");
  if (!employee.email) return [];
  const repository = await (deps.deliveryEventRepository ?? employeeDeliveryEventRepository());
  return (await repository.listByRecipient(tenantId, employee.email, 50)).map((item) => ({
    id: item.id,
...
export async function createEmployee(
  ...
  const record: EmployeeRecord = {
    ...
    ...(payload.loginEnabled
      ? { pendingRoleIds: payload.roleIds, pendingScopeType: payload.scopeType }
      : {}),
  ...
  try {
    ...
    if (!deliveryRecorded)
      await recordInviteAttempt(deps, saved, "FAILED", actor(context), now, message);
...
export async function resendEmployeeInvite(
  ...
  let knownIdentity: IdentityResult | undefined;
  try {
    knownIdentity = await deps.identityGateway.get(employee.email);
  } catch (error) {
    if (!(error instanceof Error) || error.name !== "UserNotFoundException") throw error;
  }
  if (knownIdentity?.status === "CONFIRMED") {
    ...
    return toEmployeeView(
      await completeInviteProvisioning(employee, identity, roles, now, context, deps),
    );
  }
  ...
  if (
    ...
    now.getTime() - employee.lastInviteAttemptAt.getTime() < (deps.inviteRetryCooldownMs ?? 60_000)
  ) {
  ...
  try {
    ...
    if (knownIdentity) {
      ...
      if (identity.status !== "CONFIRMED") await deps.identityGateway.resend(employee.email);
    } else {
    ...
    return toEmployeeView(
      (await repository.update(tenantId, employee.id, {
        loginStatus: "FAILED",
        inviteAttempts: employee.inviteAttempts + 1,
        lastInviteAttemptAt: now,
        inviteError: message,
...
export async function updateEmployee(
  ...
  if (
    (nextCategory === "TEACHING" && !teachingTypes.has(nextType)) ||
    (nextCategory === "NON_TEACHING" && !nonTeachingTypes.has(nextType))
  ) {
    throw new ValidationError([
      { field: "staffType", message: "staff type is not valid for the selected staff category" },
    ]);
  }
  ...
  if (!campusIds.includes(primaryCampusId)) {
    throw new ValidationError([
      { field: "primaryCampusId", message: "primary campus must be included in campus access" },
    ]);
  }
  const result = await repository.update(tenantId, employeeId, {
    ...update,
    updatedBy: actor(context),
  });
  ...
  if (current.userId && update.campusIds) {
    ...
    const activeCampusAssignments = (await assignments.listAssignments(tenantId)).filter(
      (item) =>
        item.userId === current.userId && item.isActive && item.scope.scopeType === "CAMPUS",
    );
    for (const assignment of activeCampusAssignments) {
      await assignments.updateAssignmentScope(tenantId, assignment.id, {
        scopeType: "CAMPUS",
        campusIds,
      });
    }
  ...
  if (current.userId && update.fullName) {
    await (
      await (deps.userRepository ?? userRepository())
    ).update(tenantId, current.userId, { name: update.fullName });
  }
...
export async function deactivateEmployee(
  ...
  if (employee.status === "ENDED")
    throw new ConflictError("ended employment cannot be deactivated");
  if (employee.status === "INACTIVE") return toEmployeeView(employee);
  if (employee.email && employee.loginStatus !== "NONE") {
    if (!deps.identityGateway)
      throw new Error("staff identity gateway is required to disable employee login");
    await deps.identityGateway.disable(employee.email);
  }
  if (employee.userId)
    await (
      await (deps.userRepository ?? userRepository())
    ).update(tenantId, employee.userId, { status: "INACTIVE" });
...
export async function reactivateEmployee(
  ...
  if (!employee) throw new NotFoundError("employee not found");
  if (employee.status === "ENDED")
    throw new ConflictError("ended employment cannot be reactivated");
  if (employee.status === "ACTIVE") return toEmployeeView(employee);
  if (employee.email && employee.loginStatus === "DISABLED") {
    if (!deps.identityGateway?.enable)
      throw new Error("staff identity gateway is required to enable employee login");
    await deps.identityGateway.enable(employee.email);
  }
  if (employee.userId)
    await (
      await (deps.userRepository ?? userRepository())
    ).update(tenantId, employee.userId, { status: "ACTIVE" });
  const updated = await repository.update(tenantId, employee.id, {
    status: "ACTIVE",
...
export async function activateEmployeeLogin(
  tenantId: string,
  email: string,
  deps: EmployeeServiceDeps = {},
) {
  ...
  if (!employee || employee.loginStatus === "ACTIVE")
    return employee ? toEmployeeView(employee) : null;

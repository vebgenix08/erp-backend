...
test("employee batch lookup selects only requested records from the tenant", async () => {
  const repository = new InMemoryEmployeeRepository();
  const employee = await createEmployee(base, context(), { repository });
  assert.deepEqual(
    (await repository.getMany("employee-test-tenant", [employee.id, "missing"])).map(
      (item) => item.id,
    ),
    [employee.id],
  );
  assert.deepEqual(await repository.getMany("another-tenant", [employee.id]), []);
  assert.deepEqual(await repository.getMany("employee-test-tenant", []), []);
});

test("employee resolves from a Cognito principal through its ERP user", async () => {
  const repository = new InMemoryEmployeeRepository();
  const users = new InMemoryUserRepository();
  const created = await createEmployee(base, context(), { repository });
  const user = await users.create("employee-test-tenant", {
    authUserId: "cognito-subject-one",
    email: base.email,
    name: base.fullName,
    status: "ACTIVE",
  });
  ...
  const resolved = await resolveEmployeeByPrincipal("employee-test-tenant", "cognito-subject-one", {
    repository,
...
test("employee directory applies filters and pagination in the repository", async () => {
    ...
    await createEmployee(
      {
        ...
        fullName: `Teacher ${String(index + 1).padStart(2, "0")}`,
      },
      context(),
      { repository },
    );
  const page = await listEmployeePage(
    context(),
    { repository },
    {
  ...
  assert.equal(page.items[0]?.fullName, "Teacher 03");
  assert.equal(page.summary.teaching, 5);
});

test("employee can be deactivated and reactivated with Cognito login state", async () => {
  const repository = new InMemoryEmployeeRepository();
  const created = await createEmployee(base, context(), { repository });
  await repository.update("employee-test-tenant", created.id, {
    loginStatus: "ACTIVE",
    email: "ananya.rao@example.com",
  });
  let disabled = false,
  ...
  const gateway: StaffIdentityGateway = {
    invite: async () => ({ username: "staff" }),
    resend: async () => {},
    get: async () => ({ username: "staff" }),
    disable: async () => {
      disabled = true;
    },
    enable: async () => {
  ...
  const inactive = await deactivateEmployee(created.id, context(), {
    repository,
    identityGateway: gateway,
  });
  assert.equal(inactive.status, "INACTIVE");
  assert.equal(inactive.loginStatus, "DISABLED");
  assert.equal(disabled, true);
  const active = await reactivateEmployee(created.id, context(), {
    repository,
    identityGateway: gateway,
  });
  assert.equal(active.status, "ACTIVE");
  assert.equal(active.loginStatus, "ACTIVE");
...
test("employee update rejects a primary campus outside campus access", async () => {
  ...
  await assert.rejects(
    updateEmployee(
      created.id,
      { primaryCampusId: "campus-east", campusIds: ["campus-main"] },
      context(),
      { repository },
    ),
...
test("employee campus changes synchronize active campus-scoped role assignments", async () => {
  ...
  await accessRepository.assignRole("employee-test-tenant", "erp-user-1", "teacher-role", {
    scopeType: "CAMPUS",
    campusIds: ["campus-main"],
  });
  await updateEmployee(
    created.id,
    { primaryCampusId: "campus-east", campusIds: ["campus-main", "campus-east"] },
    context(),
...
test("invite failure preserves employee for recovery", async () => {
  ...
  const result = await createEmployee(
    { ...base, loginEnabled: true, roleIds: [role!.id] },
    context(),
...
test("failed staff invite can be resent", async () => {
  ...
  const resent = await resendEmployeeInvite(failed.id, context(), {
    repository,
    identityGateway: gateway,
...
test("retry reconciles an already confirmed Cognito user without resending an invite", async () => {
  const repository = new InMemoryEmployeeRepository();
  const accessRepository = new InMemoryAccessRepository();
  const users = new InMemoryUserRepository();
  const role = (await listRoles(context().tenantContext)).find((item) => item?.code === "TEACHER")!;
  ...
  const gateway: StaffIdentityGateway = {
    invite: async () => {
      throw new Error("ERP identity write did not complete");
    },
    get: async () => ({
      username: "confirmed-teacher@example.com",
      subject: "confirmed-cognito-subject",
      tenantId: "employee-test-tenant",
      roleCode: "TEACHER",
      status: "CONFIRMED",
    }),
    resend: async () => {
  ...
  const failed = await createEmployee(
    {
      ...base,
      email: "confirmed-teacher@example.com",
      loginEnabled: true,
      roleIds: [role.id],
    },
    context(),
    { repository, identityGateway: gateway, accessRepository, userRepository: users },
  );
  await repository.update("employee-test-tenant", failed.id, {
    inviteAttempts: 5,
  ...
  assert.equal(recovered.loginStatus, "ACTIVE");
  assert.ok(recovered.userId);
  assert.equal((await users.list("employee-test-tenant"))[0]?.status, "ACTIVE");
});

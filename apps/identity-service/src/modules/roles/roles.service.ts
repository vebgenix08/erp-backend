...
function resolveRepository(deps?: RoleServiceDeps) {
  return deps?.repository ?? (defaultRepository ??= createRoleRepository());
}
...
const SYSTEM_ROLES = [
  ...
  ["VICE_PRINCIPAL", "Vice Principal", "Delegated institution oversight and academic approvals"],
  ["DEAN", "Dean", "Faculty or school leadership and academic oversight"],
  ["HOD", "Head of Department", "Assigned academic program oversight"],
  ["ACADEMIC_COORDINATOR", "Academic Coordinator", "Cross-unit academic operations and readiness"],
...
export async function listRolePage(
  ...
  filter: import("./roles.model").RolePageFilter = {},
  deps?: RoleServiceDeps,
) {
  const repository = await resolveRepository(deps),
    tenantId = resolveTenantId(context);
  await ensureSystemRoles(tenantId, repository);
  const page = await repository.listPage(tenantId, filter);
  return { ...page, items: page.items.map((role) => toRoleView(role)) };
}
...
export async function createRole(
  ...
  return toRoleView(
    await repository.create(resolveTenantId(context), { ...payload, isSystemRole: false }),
  );
}
...
export async function updateRole(
  ...
  if (
    ...
    (input.code !== undefined || input.isSystemRole !== undefined || input.isActive === false)
  ) {

...
export function createEnquiryRouter(
  ...
  router.route("GET", "/enquiries", async (context) => {
    const result = await listEnquiriesUseCase(
      ...
      deps,
      validateEnquiryListFilter(context.query),
    );
    return jsonResponse(200, result);
  });

  router.route("GET", "/enquiries/:id", async (context) => {
    const result = await getEnquiryUseCase(
      resolveEnquiryId(context),
      toServiceContext(context),
      deps,
    );
  ...
  router.route("PUT", "/enquiries/:id", async (context) => {
    const result = await updateEnquiryUseCase(
      resolveEnquiryId(context),
      context.body,
      toServiceContext(context),
      deps,
  ...
  router.route("POST", "/enquiries/:id/close", async (context) => {
    const result = await closeEnquiryUseCase(
      resolveEnquiryId(context),
      toServiceContext(context),
      deps,
    );

...
import type {
  EnquiryCreateStoredInput,
  EnquiryListFilter,
  EnquiryPage,
  EnquiryRecord,
  EnquiryUpdateStoredInput,
} from "./enquiry.model";

export interface EnquiryRepository
  extends Omit<
    TenantScopedRepository<EnquiryRecord, EnquiryCreateStoredInput, EnquiryUpdateStoredInput>,
    "list"
  > {
  ...
  close(
    tenantId: string,
    id: string,
    input: { status: "CLOSED"; closedAt: Date; updatedAt: Date },
...
function sortByCreatedAt(left: EnquiryRecord, right: EnquiryRecord): number {
  return (
    ...
    left.enquiryNumber.localeCompare(right.enquiryNumber)
  );
}
...
function normalizeFilter(filter?: EnquiryListFilter): EnquiryListFilter {
  return {
    ...
    academicTargetId: filter?.academicTargetId,
...
export class InMemoryEnquiryRepository implements EnquiryRepository {
  ...
  async listPage(tenantId: string, filter?: EnquiryListFilter) {
    ...
    const rows = [...this.getBucket(tenantId).enquiries.values()]
      .filter((enquiry) => {
        if (normalizedFilter.campusId && enquiry.campusId !== normalizedFilter.campusId)
          return false;
        if (
          normalizedFilter.academicYearId &&
          enquiry.academicYearId !== normalizedFilter.academicYearId
        )
          return false;
        if (
          normalizedFilter.academicTargetId &&
          enquiry.academicTargetId !== normalizedFilter.academicTargetId
        )
          return false;
        if (normalizedFilter.status && enquiry.status !== normalizedFilter.status) return false;
        if (
          normalizedFilter.source &&
          enquiry.source?.trim().toLowerCase() !== normalizedFilter.source
        )
          return false;
        if (
          normalizedFilter.createdFrom &&
          enquiry.createdAt.getTime() < normalizedFilter.createdFrom.getTime()
        )
          return false;
        if (
          normalizedFilter.createdTo &&
          enquiry.createdAt.getTime() > normalizedFilter.createdTo.getTime()
        )
          return false;
  ...
  async getById(tenantId: string, id: string) {
    return this.getBucket(tenantId).enquiries.get(id)
      ? clone(this.getBucket(tenantId).enquiries.get(id) as EnquiryRecord)
      : null;
  }

  async getByEnquiryNumber(tenantId: string, enquiryNumber: string) {
    const record = [...this.getBucket(tenantId).enquiries.values()].find(
      (item) => item.enquiryNumber === enquiryNumber,
    );
...
interface EnquiryDocument extends EnquiryRecord {
  _id: string;
}
...
const toDocument = (record: EnquiryRecord): EnquiryDocument => ({
  ...clone(record),
  _id: record.id,
});
const fromDocument = (document: EnquiryDocument | null): EnquiryRecord | null => {
...
class MongoEnquiryRepository implements EnquiryRepository {
  constructor(
    private collection: CollectionAdapter<EnquiryDocument>,
    private sequences: CollectionAdapter<SequenceDocument>,
  ) {}
  async nextEnquirySequence(tenantId: string) {
    const owner = requiredTenant(tenantId);
    const result = await this.sequences.findOneAndUpdate(
      { tenantId: owner, _id: `enquiry:${owner}` },
  ...
  async listPage(tenantId: string, filter?: EnquiryListFilter) {
    ...
    const query: TenantFilter<EnquiryDocument> = {
      tenantId: requiredTenant(tenantId),
    };
    if (normalized.campusId) query.campusId = normalized.campusId;
    if (normalized.academicYearId) query.academicYearId = normalized.academicYearId;
    if (normalized.academicTargetId) query.academicTargetId = normalized.academicTargetId;
    if (normalized.status) query.status = normalized.status;
    if (normalized.createdFrom || normalized.createdTo) {
      query.createdAt = {
        ...(normalized.createdFrom ? { $gte: normalized.createdFrom } : {}),
        ...(normalized.createdTo ? { $lte: normalized.createdTo } : {}),
      };
    }
    if (normalized.source) {
      query.source = {
        $regex: `^${normalized.source.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`,
        $options: "i",
      };
    }
    if (normalized.search) {
      query.$or = [
        ...
        [field]: { $regex: normalized.search, $options: "i" },
      }));
    }
    ...
    const [documents, total] = await Promise.all([
      this.collection.findMany(query, {
        sort: { createdAt: 1, enquiryNumber: 1 },
        skip: offset,
        limit,
      }),
    ...
    return {
      items: documents
        .map(fromDocument)
        .filter((record): record is EnquiryRecord => Boolean(record)),
      total,
      limit,
      offset,
    };
  ...
  async create(tenantId: string, input: EnquiryCreateStoredInput) {
    const normalized = requiredTenant(tenantId),
      record: EnquiryRecord = {
        ...
        ...(input.academicTargetId ? { academicTargetId: input.academicTargetId } : {}),
        studentName: input.studentName,
        ...
        ...(input.interestedClass ? { interestedClass: input.interestedClass } : {}),
  ...
  async update(tenantId: string, id: string, input: EnquiryUpdateStoredInput) {
    ...
    return (await this.collection.replaceOne(
      { tenantId: requiredTenant(tenantId), _id: id },
      toDocument(updated),
    ))
      ? clone(updated)
      : null;
...
export async function createEnquiryRepository(
  env: MongoEnvLike = runtimeEnv(),
): Promise<EnquiryRepository> {
  if (!hasMongo(env)) return new InMemoryEnquiryRepository();
  const collection = await getCollection<EnquiryDocument>("admissions_enquiries", env),
    sequences = await getCollection<SequenceDocument>("admissions_sequences", env);
  await collection.createIndex({ tenantId: 1, enquiryNumber: 1 }, { unique: true });
  ...
  await collection.createIndex({ tenantId: 1, academicTargetId: 1, createdAt: -1 });
  await collection.createIndex({ tenantId: 1, source: 1, createdAt: -1 });
  await collection.createIndex({ tenantId: 1, phone: 1 });
  return new MongoEnquiryRepository(
    createMongoCollectionAdapter(collection),
    createMongoCollectionAdapter(sequences),
  );
}
let singleton: Promise<EnquiryRepository> | undefined;
function defaultRepository() {
  return (singleton ??= createEnquiryRepository());
}
export const enquiryRepository: EnquiryRepository = {
  list: async (...args) => (await defaultRepository()).list(...args),
  listPage: async (...args) => (await defaultRepository()).listPage(...args),
  getById: async (...args) => (await defaultRepository()).getById(...args),
  getByEnquiryNumber: async (...args) => (await defaultRepository()).getByEnquiryNumber(...args),
  nextEnquirySequence: async (...args) => (await defaultRepository()).nextEnquirySequence(...args),
  create: async (...args) => (await defaultRepository()).create(...args),
  update: async (...args) => (await defaultRepository()).update(...args),
  close: async (...args) => (await defaultRepository()).close(...args),
};

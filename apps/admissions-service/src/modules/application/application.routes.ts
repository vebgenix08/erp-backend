...
export function createApplicationRouter(router: ApiRouter, deps: ApplicationServiceDeps = {}) {
  router.route("GET", "/applications", async (ctx) =>
    jsonResponse(200, await listApplicationsUseCase(context(ctx), deps, ctx.query)),
  );
  ...
  router.route("POST", "/applications", async (ctx) =>
    jsonResponse(201, await createApplicationUseCase(ctx.body, context(ctx), deps)),
  );
  router.route("PUT", "/applications/:id", async (ctx) =>
    jsonResponse(200, await updateApplicationUseCase(id(ctx), ctx.body, context(ctx), deps)),
  );
  router.route("POST", "/applications/:id/submit", async (ctx) =>
    jsonResponse(200, await submitApplicationUseCase(id(ctx), context(ctx), deps)),
  );
  router.route("POST", "/applications/:id/approve", async (ctx) =>
    jsonResponse(200, await approveApplicationUseCase(id(ctx), ctx.body, context(ctx), deps)),
  );
  router.route("POST", "/applications/:id/reject", async (ctx) =>
    jsonResponse(200, await rejectApplicationUseCase(id(ctx), ctx.body, context(ctx), deps)),
  );
  router.route("POST", "/applications/:id/cancel", async (ctx) =>
    jsonResponse(200, await cancelApplicationUseCase(id(ctx), ctx.body, context(ctx), deps)),
  );
  return router;
}

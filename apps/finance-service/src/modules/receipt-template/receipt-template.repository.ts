...
interface Document extends ReceiptTemplateRecord {
  _id: string;
}
...
function clone(value: ReceiptTemplateRecord) {
  return {
    ...value,
...
export class InMemoryReceiptTemplateRepository implements ReceiptTemplateRepository {
  ...
  async save(tenantId: string, actorId: string, input: ReceiptTemplateInput) {
    ...
    this.records.set(tenantId, clone(record));
    return clone(record);
  }
}
class MongoReceiptTemplateRepository implements ReceiptTemplateRepository {
  ...
  async get(tenantId: string) {
    const value = await this.collection.findOne({ tenantId });
    return value ? clone(value) : null;
  }
  async save(tenantId: string, actorId: string, input: ReceiptTemplateInput) {
    const existing = await this.get(tenantId);
    const now = new Date();
    const record: ReceiptTemplateRecord = {
      ...input,
      id: `receipt_template_${tenantId}`,
      tenantId,
      updatedBy: actorId,
...
export function receiptTemplateRepository(): Promise<ReceiptTemplateRepository> {
  singleton ??= (async () => {
    ...
    if (
      ...
      return new InMemoryReceiptTemplateRepository();
    const connection = await getMongoConnection(runtime);
    const collection = connection.client
      .db(connection.dbName)
      .collection<Document>("finance_receipt_templates");
    await collection.createIndex({ tenantId: 1 }, { unique: true });
    return new MongoReceiptTemplateRepository(createTenantMongoCollection(collection));
  })();
  return singleton;
}

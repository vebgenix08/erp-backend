...
} from "./receipt-template.repository";
...
export interface ReceiptTemplateDependencies {
  repository?: ReceiptTemplateRepository | Promise<ReceiptTemplateRepository>;
}
function tenantId(context: RequestContext) {
...
function requirePermission(context: RequestContext, permission: Permission) {
  if (!context.authContext?.user?.permissions.includes(permission))
    throw new ForbiddenError(`permission ${permission} is required`);
}
function view(record: ReturnType<typeof defaultReceiptTemplate>): ReceiptTemplateView {
  return {
    ...record,
...
async function repository(deps?: ReceiptTemplateDependencies) {
  return await (deps?.repository ?? receiptTemplateRepository());
}
export async function getReceiptTemplate(
  context: RequestContext,
  deps?: ReceiptTemplateDependencies,
) {
  requirePermission(context, receiptTemplatePermissions.read as Permission);
  const id = tenantId(context);
  return view((await (await repository(deps)).get(id)) ?? defaultReceiptTemplate(id));
}
export async function saveReceiptTemplate(
  ...
  requirePermission(context, receiptTemplatePermissions.manage as Permission);
  return view(
    await (
      await repository(deps)
    ).save(tenantId(context), actorId(context), validateReceiptTemplate(input)),
  );
}

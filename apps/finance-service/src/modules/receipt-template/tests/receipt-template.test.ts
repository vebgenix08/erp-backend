...
function context(tenantId = "tenant_one"): RequestContext {
  return {
    ...
    authContext: {
      ...
      user: {
        id: "admin_1",
        permissions: ["finance.receipt-template.read", "finance.receipt-template.manage"],
        source: "headers",
      },
    },
  };
...
test("returns a safe default and stores one tenant-isolated receipt template", async () => {
  ...
  const saved = await saveReceiptTemplate(
    {
      ...
      headerText: "Payment acknowledged",
      footerText: "Thank you",
      signatureLabel: "Accounts officer",
      paperSize: "A4",
      accentColor: "#14532d",
  ...
  assert.equal(
    (await getReceiptTemplate(context("tenant_two"), { repository })).title,
    "Fee payment receipt",
  );
});
  ...
  await assert.rejects(
    () =>
      saveReceiptTemplate(
        ...
        context(),
        { repository: new InMemoryReceiptTemplateRepository() },
      ),
    /Validation failed/,
  );
});

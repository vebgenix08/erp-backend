...
import type { FeeOrderRecoveryFilter, FeeOrderRecoveryStatus } from "./fee-order-recovery.model";
const text = (value: unknown, field: string) => {
  if (typeof value !== "string" || !value.trim()) throw new BadRequestError(`${field} is required`);
  return value.trim();
};
export function validateFeeOrderRecoveryFilter(input: unknown): FeeOrderRecoveryFilter {
  ...
  if (value.status !== undefined) {
    const status = text(value.status, "status").toUpperCase() as FeeOrderRecoveryStatus;

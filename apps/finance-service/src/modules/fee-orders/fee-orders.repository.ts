...
export interface FeeOrderRepository {
  create(tenantId: string, record: FeeOrderCreateInput): Promise<FeeOrderRecord>;
  getById(tenantId: string, id: string): Promise<FeeOrderRecord | null>;
  getByEnrollmentId(tenantId: string, enrollmentId: string): Promise<FeeOrderRecord | null>;
  ...
  replace(tenantId: string, record: FeeOrderRecord): Promise<FeeOrderRecord | null>;
}
...
export class InMemoryFeeOrderRepository implements FeeOrderRepository {
  ...
  async create(tenantId: string, input: FeeOrderCreateInput) {
  ...
  async getByEnrollmentId(tenantId: string, enrollmentId: string) {
    ...
    const record = [...this.records.values()].find(
      (item) => item.tenantId === normalizedTenant && item.enrollmentId === enrollmentId,
    );
  ...
  async getByStudentAcademicYear(tenantId: string, studentId: string, academicYearId: string) {
    ...
    const record = [...this.records.values()].find(
      (item) =>
        ...
        (item.sourceType ?? "ANNUAL") === "ANNUAL",
    );
    return record ? clone(record) : null;
  }
  async getBySourceStudent(
    tenantId: string,
    sourceType: FeeOrderRecord["sourceType"],
    sourceId: string,
    studentId: string,
    ...
    const record = [...this.records.values()].find(
      (item) =>
        item.tenantId === normalizedTenant &&
        (item.sourceType ?? "ANNUAL") === sourceType &&
        (item.sourceId ?? item.enrollmentId) === sourceId &&
        item.studentId === studentId,
    );
    return record ? clone(record) : null;
  }
  async list(tenantId: string, filter: FeeOrderFilter = {}) {
    return [...this.records.values()]
      .filter((item) => item.tenantId === tenant(tenantId) && matches(item, filter))
...
class MongoFeeOrderRepository implements FeeOrderRepository {
  ...
  async getByStudentAcademicYear(tenantId: string, studentId: string, academicYearId: string) {
    const document = await this.orders.findOne({
      ...
      $or: [{ "record.sourceType": "ANNUAL" }, { "record.sourceType": { $exists: false } }],
    });
    return document ? clone(document.record) : null;
  }
  async getBySourceStudent(
  ...
  async list(tenantId: string, filter: FeeOrderFilter = {}) {
    const query: TenantFilter<FeeOrderDocument> = { tenantId: tenant(tenantId) };
    ...
    return (await this.orders.findMany(query, { sort: { "record.createdAt": -1 } })).map((item) =>
      clone(item.record),
    );
  }
  async listPage(tenantId: string, filter: FeeOrderFilter = {}) {
...
export async function createFeeOrderRepository(
  ...
  const orders = await getCollection<FeeOrderDocument>("finance_fee_orders", env);
  await orders.createIndex({ tenantId: 1, "record.enrollmentId": 1 }, { unique: true });
  await orders.createIndex({ tenantId: 1, "record.orderNumber": 1 }, { unique: true });
  await orders
    .dropIndex("tenantId_1_record.studentId_1_record.academicYearId_1")
    .catch(() => undefined);
  ...
  await orders.createIndex(
    ...
    {
      name: "student_annual_fee_order_history",
      partialFilterExpression: { "record.sourceType": "ANNUAL" },
    },
  );
  await orders.createIndex(
    ...
    {
      unique: true,
      name: "uq_fee_order_source_student",
      partialFilterExpression: { "record.sourceType": { $exists: true } },
    },
  );
  await orders.createIndex({
    tenantId: 1,
  ...
  return new MongoFeeOrderRepository(createMongoCollectionAdapter(orders), env);
}

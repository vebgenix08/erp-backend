...
export function validateFeeOrderFilter(value: unknown): FeeOrderFilter {
  ...
  for (const key of [
    ...
    if (input[key] !== undefined) {
      if (typeof input[key] !== "string" || !input[key].trim())
        throw new ValidationError([{ field: key, message: `${key} must be a non-empty string` }]);
  ...
  if (input.status !== undefined) {
    ...
    result.status = input.status as Exclude<FeeOrderFilter["status"], undefined>;
  }
  ...
  for (const field of ["limit", "offset"] as const) {
    ...
    if (typeof input[field] !== "number" || !Number.isSafeInteger(input[field]) || input[field] < 0)
      throw new ValidationError([{ field, message: `${field} must be a non-negative integer` }]);
    result[field] = input[field];
  }
  if ((result.limit ?? 100) > 200)
    throw new ValidationError([{ field: "limit", message: "limit cannot exceed 200" }]);
  return result;
}

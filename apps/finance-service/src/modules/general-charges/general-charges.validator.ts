...
export function validateCreateGeneralCharge(input: unknown): CreateGeneralChargeInput {
  ...
  if (!Number.isSafeInteger(value.amountMinor) || (value.amountMinor as number) <= 0)
    throw new ValidationError([
      { field: "amountMinor", message: "amountMinor must be a positive integer" },
    ]);
  const policy = text(
    value.collectionPolicy,
    "collectionPolicy",
  ) as CreateGeneralChargeInput["collectionPolicy"];
  if (!["FULL_ONLY", "PARTIAL_ALLOWED"].includes(policy))
    throw new ValidationError([
      { field: "collectionPolicy", message: "collectionPolicy is invalid" },
    ]);
  ...
  if (!Array.isArray(target.ids) || !target.ids.length || target.ids.length > 500)
    throw new ValidationError([
      { field: "target.ids", message: "target ids must contain between 1 and 500 values" },
    ]);
...
export function validateGeneralChargeFilter(input: unknown): GeneralChargeFilter {
  ...
  if (value.academicYearId !== undefined)
    result.academicYearId = text(value.academicYearId, "academicYearId");

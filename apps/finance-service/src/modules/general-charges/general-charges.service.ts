...
import {
  ...
  type GeneralChargeRepository,
} from "./general-charges.repository";
import {
  validateCreateGeneralCharge,
  validateGeneralChargeFilter,
} from "./general-charges.validator";
...
const charges = async (deps?: GeneralChargeDependencies) =>
  await (deps?.charges ?? generalChargeRepository());
const orders = async (deps?: GeneralChargeDependencies) =>
  await (deps?.orders ?? feeOrderRepository());
const configuration = async (deps?: GeneralChargeDependencies) =>
  await (deps?.configuration ?? feeConfigurationRepository());

function eligibleStudents(
  annualOrders: FeeOrderRecord[],
  ...
  if (target.type === "STUDENT") {
    ...
    if (missing.length)
      throw new NotFoundError(
        `${missing.length} selected students do not have an annual finance projection`,
      );
  }
  if (!unique.size)
    throw new NotFoundError("no eligible students were found for the selected target");
  return [...unique.values()];
}

export async function createGeneralCharge(
  input: unknown,
  ...
  ).snapshot(tenant, { campusId: value.campusId, academicYearId: value.academicYearId });
  const feeHead = snapshot.feeHeads.find(
    (item) => item.id === value.feeHeadId && item.status === "ACTIVE",
  );
  ...
  try {
    ...
    assignment = await repository.update(tenant, {
    ...
    await repository.update(tenant, {
      ...assignment,
      status: "FAILED",
      failureReason: error instanceof Error ? error.message : "general charge assignment failed",
      updatedAt: deps.now?.() ?? new Date(),
    });
...
export async function listGeneralCharges(
  ...
  return (
    await (await charges(deps)).list(tenantId(context), validateGeneralChargeFilter(filter))
  ).map(toGeneralChargeView);
}

...
export function registerPaymentRoutes(router: ApiRouter, deps: PaymentDependencies = {}) {
  ...
  router.route("GET", "/receipts/:paymentId", async (context: RequestContext) =>
    jsonResponse(200, await getReceipt(context.params.paymentId ?? "", context, deps)),
  );
  return router;
}

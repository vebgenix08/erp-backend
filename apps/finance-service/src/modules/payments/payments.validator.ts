...
const methods: PaymentMethod[] = ["CASH", "CARD", "UPI", "BANK_TRANSFER", "CHEQUE", "ONLINE"];
const statuses: PaymentStatus[] = ["SUCCESS", "PARTIALLY_REFUNDED", "VOIDED", "REFUNDED"];
...
export function validateCollectPayment(input: unknown): CollectPaymentInput {
  ...
  if (!methods.includes(method)) throw new BadRequestError("payment method is invalid");
  ...
  const allocations = value.allocations.map((item, index) => {
    ...
    const allocation = {
      ...
      amountMinor: positive(row.amountMinor, `allocations[${index}].amountMinor`),
    };
    if (row.chargeAllocations === undefined) return allocation;
    if (!Array.isArray(row.chargeAllocations) || !row.chargeAllocations.length)
      throw new BadRequestError(`allocations[${index}].chargeAllocations must not be empty`);
    ...
    if (
      new Set(chargeAllocations.map((charge) => charge.chargeId)).size !== chargeAllocations.length
    )
      throw new BadRequestError(`allocations[${index}] contains a duplicate charge allocation`);
    if (
  ...
  if (new Set(allocations.map((item) => item.feeOrderId)).size !== allocations.length)
    throw new BadRequestError("a fee order can be allocated only once per payment");
  ...
  if (typeof value.paidAt === "string" && value.paidAt.trim()) {
    if (Number.isNaN(Date.parse(value.paidAt))) throw new BadRequestError("paidAt is invalid");
...
export function validatePaymentFilter(input: unknown): PaymentFilter {
  ...
  for (const key of ["campusId", "academicYearId", "studentId", "search"] as const)
  ...
  for (const field of ["paidFrom", "paidTo"] as const) {
    ...
    if (Number.isNaN(Date.parse(date))) throw new BadRequestError(`${field} must be a valid date`);
    result[field] = date;
  }
  if (result.paidFrom && result.paidTo && Date.parse(result.paidFrom) > Date.parse(result.paidTo))
  ...
  if (value.status !== undefined) {
    ...
    if (!statuses.includes(status)) throw new BadRequestError("payment status is invalid");
  ...
  if ((result.limit ?? 100) > 200) throw new BadRequestError("limit cannot exceed 200");
  return result;
}

...
import {
  ...
  resolveFeeOrderRecoveryForEvent,
} from "../modules/fee-order-recovery/fee-order-recovery.service";

interface EventBridgeEnvelope {
  detail: StudentEnrolledEvent | StudentEnrollmentChangedEvent;
}
...
export async function handler(event: EventBridgeEnvelope): Promise<void> {
  ...
  if (
    !["academics.student.enrolled.v1", "academics.student.enrollment-changed.v1"].includes(
      domainEvent.type,
    ) ||
    domainEvent.source !== "erp.academics"
  )
    throw new Error("unsupported academics event");
  try {
    await generateFeeOrderFromEnrollment(domainEvent.data, domainEvent.tenantId);
    await resolveFeeOrderRecoveryForEvent(

...
import {
  ...
  listFeeOrderPage,
  listFeeOrders,
} from "../modules/fee-orders/fee-orders.service";
import {
  collectPayment,
  getReceipt,
  listPaymentPage,
...
import {
  ...
  updateFeeHead,
} from "../modules/fee-configuration/fee-configuration.service";
...
} from "../modules/payment-adjustments/payment-adjustments.service";
...
} from "../modules/fee-order-recovery/fee-order-recovery.service";
import { receiptTemplatePermissions } from "../modules/receipt-template/receipt-template.permissions";
import {
  getReceiptTemplate,
  saveReceiptTemplate,
} from "../modules/receipt-template/receipt-template.service";
...
const adminPermissions = [
  ...Object.values(financeDashboardPermissions),
  ...Object.values(feeConfigurationPermissions),
  ...Object.values(feeOrderPermissions),
  ...Object.values(generalChargePermissions),
  ...Object.values(paymentPermissions),
  ...Object.values(paymentAdjustmentPermissions),
  ...Object.values(feeOrderRecoveryPermissions),
  ...Object.values(receiptTemplatePermissions),
];
...
function context(event: Event): RequestContext {
  ...
  const permissions = normalizePermissions([
    ...(role === "TENANT_ADMIN" ? adminPermissions : []),
...
function requiredText(args: Record<string, unknown>, name: string) {
  const value = args[name];
...
export async function handleFinanceGraphql(event: Event) {
  const ctx = context(event);
  await hydrateConfiguredAuthorization(ctx);
  const args = event.arguments ?? {};
  switch (event.info.fieldName) {
    case "feeConfiguration":
      return listFeeConfiguration(requiredObject(args, "scope"), ctx);
    case "financeDashboard":
      return getFinanceDashboard(requiredObject(args, "scope"), ctx);
    case "financeReceipt":
      return getReceipt(requiredText(args, "paymentId"), ctx);
    case "financeReceiptTemplate":
      return getReceiptTemplate(ctx);
    case "financePayments":
      return listPayments(args.filter, ctx);
    case "financePaymentPage":
      return listPaymentPage(args.filter, ctx);
    case "financePaymentAdjustments":
      return listPaymentAdjustments(args.filter, ctx);
    case "feeOrderRecoveries":
      return listFeeOrderRecoveries(args.filter, ctx);
    case "feeOrders":
      return listFeeOrders(args.filter, ctx);
    case "feeOrderPage":
      return listFeeOrderPage(args.filter, ctx);
    case "generalCharges":
      return listGeneralCharges(args.filter, ctx);
    case "feeOrder":
      return getFeeOrder(requiredText(args, "id"), ctx);
    case "createFeeHead":
      return createFeeHead(requiredObject(args, "input"), ctx);
    case "updateFeeHead":
      return updateFeeHead(requiredText(args, "id"), requiredObject(args, "input"), ctx);
    case "createFeeSchedule":
      return createFeeSchedule(requiredObject(args, "input"), ctx);
    case "createFeeStructure":
      return createFeeStructure(requiredObject(args, "input"), ctx);
    case "createFeeMapping":
      return createFeeMapping(requiredObject(args, "input"), ctx);
    case "setFeeConfigurationStatus":
      return setFeeConfigurationStatus(
        requiredText(args, "entity") as "fee-head" | "schedule" | "structure" | "mapping",
        requiredText(args, "id"),
        requiredText(args, "status") as "ACTIVE" | "INACTIVE",
        ctx,
      );
    case "collectFinancePayment":
    ...
    case "createFinancePaymentAdjustment":
      return createPaymentAdjustment(requiredObject(args, "input"), ctx);
    case "retryFeeOrderRecovery":
      return retryFeeOrderRecovery(requiredText(args, "id"), ctx);
    default:
      throw new NotFoundError(`unsupported finance GraphQL field: ${event.info.fieldName}`);
  }
}
export async function handler(event: Event) {
  try {
    ...
    if (event.source === "erp.internal" && event.operation === "GET_ADMIN_DASHBOARD_SLICE") {
      ...
      return {
        result: await readFinanceDashboardSlice(tenantId.trim(), scope as never),
      };
    }
    return await handleFinanceGraphql(event);
  } catch (error) {

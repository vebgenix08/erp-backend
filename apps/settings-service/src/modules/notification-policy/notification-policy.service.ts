...
import {
  ...
  type NotificationPolicyRepository,
...
function tenantId(context: Context) {
  requireAuth(context.authContext);
  return requireTenantId(context.tenantContext);
}
export async function getNotificationPolicy(
  context: Context,
  deps: { repository?: NotificationPolicyRepository } = {},
) {
  requirePermission(context.authContext, notificationPolicyPermissions.read);
  const tenant = tenantId(context);
  return toNotificationPolicyView(
...
export async function updateNotificationPolicy(
  ...
  requirePermission(context.authContext, notificationPolicyPermissions.update);
  return toNotificationPolicyView(
    await (deps.repository ?? notificationPolicyRepository).save(
      tenantId(context),
      validateNotificationPolicy(input),
    ),
  );
}

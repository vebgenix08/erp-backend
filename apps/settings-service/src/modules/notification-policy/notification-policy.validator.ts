...
function optionalEmail(value: unknown, field: string): string | undefined {
  ...
  if (normalized && !emailPattern.test(normalized))
    throw new BadRequestError(`${field} must be a valid email`);
  return normalized || undefined;
}

export function validateNotificationPolicy(input: unknown): NotificationPolicyInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("notification policy input is required");
  ...
  const mapped = value.events.map((item) => {
    if (!item || typeof item !== "object" || Array.isArray(item))
      throw new BadRequestError("notification event is invalid");
    ...
    if (!events.has(event) || seen.has(event))
      throw new BadRequestError("notification event is invalid or duplicated");
    if (!audiences.has(audience)) throw new BadRequestError("notification audience is invalid");
    seen.add(event);
    return {
      event,
      label: DEFAULT_NOTIFICATION_EVENTS.find((entry) => entry.event === event)?.label ?? event,
      audience,
      email: eventValue.email === true,
      sms: eventValue.sms === true,
    };
  });
  if (mapped.length !== DEFAULT_NOTIFICATION_EVENTS.length)
    throw new BadRequestError("all supported notification events are required");
  return {

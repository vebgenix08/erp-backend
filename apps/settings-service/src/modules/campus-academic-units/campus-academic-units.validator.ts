...
export function validateAcademicUnitCreateInput(input: unknown): CampusAcademicUnitCreateInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("academic unit input is required");
  const value = input as Record<string, unknown>;
  const name = text(value.name);
  const curriculumOrAffiliationId = text(value.curriculumOrAffiliationId);
  if (!name) throw new BadRequestError("academic unit name is required");
  if (!curriculumOrAffiliationId)
    throw new BadRequestError("curriculum or affiliation is required");
...
export function validateAcademicUnitUpdateInput(input: unknown): CampusAcademicUnitUpdateInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("academic unit update input is required");
  const value = input as Record<string, unknown>;
  const result: CampusAcademicUnitUpdateInput = {};
  if (value.name !== undefined) {
    result.name = text(value.name);
    if (!result.name) throw new BadRequestError("academic unit name cannot be empty");
  }
  if (value.curriculumOrAffiliationId !== undefined) {
    result.curriculumOrAffiliationId = text(value.curriculumOrAffiliationId);
    if (!result.curriculumOrAffiliationId)
      throw new BadRequestError("curriculum or affiliation cannot be empty");
  }
...
export function validateAcademicUnitListFilter(input: unknown): CampusAcademicUnitListFilter {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("academic unit filter must be an object");
  const value = input as Record<string, unknown>;
  const result: CampusAcademicUnitListFilter = {};

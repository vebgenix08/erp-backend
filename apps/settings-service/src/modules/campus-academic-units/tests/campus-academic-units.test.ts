...
test("one campus can have two school units with different curricula", async () => {
  ...
  await repository.create("tenant_one", "campus_main", {
    name: "State School",
    type: "SCHOOL",
    curriculumOrAffiliationId: "STATE_BOARD",
  });
  assert.equal(
    (await repository.list("tenant_one", { campusId: "campus_main", type: "SCHOOL" })).length,
    2,
  );
  await assert.rejects(
    () =>
      repository.create("tenant_one", "campus_main", {
        name: "Duplicate CBSE Unit",
        type: "SCHOOL",
        curriculumOrAffiliationId: "CBSE",
      }),
    /already exists/,

...
import {
  campusAcademicUnitRepository,
  type CampusAcademicUnitRepository,
} from "./campus-academic-units.repository";
import {
  validateAcademicUnitCreateInput,
  validateAcademicUnitListFilter,
  validateAcademicUnitUpdateInput,
} from "./campus-academic-units.validator";
...
const ensure = (context: RequestContext, permission: string) => {
  requireAuth(context.authContext);
  requirePermission(context.authContext, permission);
};
const view = (
  record: Awaited<ReturnType<CampusAcademicUnitRepository["create"]>>,
): CampusAcademicUnitView => ({
  ...record,
  createdAt: record.createdAt.toISOString(),
  updatedAt: record.updatedAt.toISOString(),
  deactivatedAt: record.deactivatedAt?.toISOString(),
});
export async function listCampusAcademicUnits(
  context: RequestContext,
  filter?: unknown,
  deps?: CampusAcademicUnitServiceDeps,
) {
  ensure(context, campusAcademicUnitPermissions.read);
  return (
    await (deps?.repository ?? campusAcademicUnitRepository).list(
      tenant(context),
      validateAcademicUnitListFilter(filter),
    )
  ).map(view);
}
export async function createCampusAcademicUnit(
  context: RequestContext,
  campusId: string,
  input: unknown,
  deps?: CampusAcademicUnitServiceDeps,
) {
  ensure(context, campusAcademicUnitPermissions.create);
  ...
  return view(
    await (deps?.repository ?? campusAcademicUnitRepository).create(
      tenantId,
      campusId,
      validateAcademicUnitCreateInput(input),
    ),
  );
}
export async function updateCampusAcademicUnit(
  context: RequestContext,
  id: string,
  input: unknown,
  deps?: CampusAcademicUnitServiceDeps,
  ...
  const record = await (deps?.repository ?? campusAcademicUnitRepository).update(
    tenant(context),
    id,
    validateAcademicUnitUpdateInput(input),
  );

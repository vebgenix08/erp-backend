...
import type {
  ...
  CampusAcademicUnitListFilter,
  CampusAcademicUnitRecord,
  CampusAcademicUnitUpdateInput,
} from "./campus-academic-units.model";

export interface CampusAcademicUnitRepository {
  list(
    tenantId: string,
    filter?: CampusAcademicUnitListFilter,
  ): Promise<CampusAcademicUnitRecord[]>;
  getById(tenantId: string, id: string): Promise<CampusAcademicUnitRecord | null>;
  create(
    tenantId: string,
    campusId: string,
    input: CampusAcademicUnitCreateInput,
  ): Promise<CampusAcademicUnitRecord>;
  update(
    tenantId: string,
    id: string,
    input: CampusAcademicUnitUpdateInput,
  ): Promise<CampusAcademicUnitRecord | null>;
}
interface Document extends CampusAcademicUnitRecord {
  _id: string;
  unitKey: string;
}
const unitKey = (campusId: string, type: string, affiliationId: string) =>
  `${campusId}:${type}:${affiliationId}`.toLowerCase();
const clone = (record: CampusAcademicUnitRecord): CampusAcademicUnitRecord => ({
  ...record,
...
const fromDocument = (document: Document | null) => {
  ...
  const { _id, unitKey: _, ...record } = document;
  return clone({ ...record, id: record.id || _id });
};
const toDocument = (record: CampusAcademicUnitRecord): Document => ({
  ...clone(record),
  _id: record.id,
  unitKey: unitKey(record.campusId, record.type, record.curriculumOrAffiliationId),
});
  ...
  async create(tenantId: string, campusId: string, input: CampusAcademicUnitCreateInput) {
    if (
      [...this.records.values()].some(
        (item) =>
          item.tenantId === tenantId &&
          unitKey(item.campusId, item.type, item.curriculumOrAffiliationId) ===
            unitKey(campusId, input.type, input.curriculumOrAffiliationId),
      )
    )
      throw new ConflictError("academic unit already exists for this campus and affiliation");
    const id = `unit_${crypto.randomUUID()}`;
    const timestamp = new Date();
    const record: CampusAcademicUnitRecord = {
      id,
      tenantId,
      campusId,
  ...
  async update(tenantId: string, id: string, input: CampusAcademicUnitUpdateInput) {
    ...
    const next = clone({
      ...
      curriculumOrAffiliationId:
        input.curriculumOrAffiliationId ?? current.curriculumOrAffiliationId,
      status,
      updatedAt: new Date(),
...
class MongoCampusAcademicUnitRepository implements CampusAcademicUnitRepository {
  ...
  async list(tenantId: string, filter: CampusAcademicUnitListFilter = {}) {
    ...
    return (await this.collection.findMany(query))
      .map(fromDocument)
      .filter((item): item is CampusAcademicUnitRecord => Boolean(item));
  }
  async getById(tenantId: string, id: string) {
    return fromDocument(await this.collection.findOne({ tenantId, _id: id }));
  }
  async create(tenantId: string, campusId: string, input: CampusAcademicUnitCreateInput) {
    const key = unitKey(campusId, input.type, input.curriculumOrAffiliationId);
    if (await this.collection.findOne({ tenantId, unitKey: key }))
      throw new ConflictError("academic unit already exists for this campus and affiliation");
    const id = `unit_${crypto.randomUUID()}`;
    const timestamp = new Date();
    const record: CampusAcademicUnitRecord = {
  ...
  async update(tenantId: string, id: string, input: CampusAcademicUnitUpdateInput) {
    ...
    await this.collection.replaceOne({ tenantId, _id: id }, toDocument(next));
    return next;
  }
}
const env = (): MongoEnvLike =>
  (globalThis as unknown as { process?: { env?: MongoEnvLike } }).process?.env ?? {};
export async function createCampusAcademicUnitRepository(
  runtime = env(),
): Promise<CampusAcademicUnitRepository> {
  if (
    !runtime.MONGODB_URI &&
    ...
    return new InMemoryCampusAcademicUnitRepository();

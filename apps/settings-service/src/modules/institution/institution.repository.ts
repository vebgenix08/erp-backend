...
export interface InstitutionRepository {
  ...
  ): Promise<InstitutionProfileRecord | null>;
  create(
    tenantId: string,
    input: InstitutionProfileInput,
    context?: RepositoryContext,
  ): Promise<InstitutionProfileRecord>;
  update(
    tenantId: string,
    id: string,
    input: InstitutionProfileUpdateInput,
    context?: RepositoryContext,
  ): Promise<InstitutionProfileRecord | null>;
}
...
interface InstitutionDocument extends InstitutionProfileRecord {
  _id: string;
}
...
export class MongoInstitutionRepository implements InstitutionRepository {
  ...
  async list(tenantId: string) {
    const row = await this.getById(tenantId, "institution");
    return row ? [row] : [];
  }
  async getById(tenantId: string, _id: string) {
    const row = await this.collection.findOne({ _id: `institution_${tenantId}`, tenantId });
    if (!row) return null;
    const { _id: _ignored, ...record } = row;
    return clone(record);
  }
  async create(tenantId: string, input: InstitutionProfileInput) {
    const timestamp = now();
    const record: InstitutionProfileRecord = {
      id: `institution_${tenantId}`,
      tenantId,
    ...
    await this.collection.insertOne({ _id: record.id, ...record });
    return clone(record);
  }
  async update(tenantId: string, _id: string, input: InstitutionProfileUpdateInput) {
    const existing = await this.getById(tenantId, "institution");
    if (!existing) return null;
    const updated = { ...existing, ...input, updatedAt: now() };
    await this.collection.replaceOne(
      { _id: existing.id, tenantId },
      { _id: existing.id, ...updated },
    );
    return clone(updated);
  }
}
...
export async function createInstitutionRepository(
  env: MongoEnvLike = runtimeEnv(),
): Promise<InstitutionRepository> {
  if (!env.MONGODB_URI && !env.MONGODB_URI_DEV && !env.MONGODB_URI_PROD && !env.MONGODB_URI_TEST)
    return new InMemoryInstitutionRepository();
  const collection = await getCollection<InstitutionDocument>("settings_institution_profiles", env);
  await collection.createIndex({ tenantId: 1 }, { unique: true });
  return new MongoInstitutionRepository(createMongoCollectionAdapter(collection));
}
async function getDefaultRepository() {
  defaultRepository ??= createInstitutionRepository();

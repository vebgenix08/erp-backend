...
export interface CampusRepository {
  ...
  create(
    tenantId: string,
    input: CampusCreateInput & { code: string },
    context?: RepositoryContext,
  ): Promise<CampusRecord>;
...
export class InMemoryCampusRepository implements CampusRepository {
  ...
  async list(tenantId: string, filter?: CampusListFilter) {
    ...
    return records
      ...
      .filter(
        (record) =>
          !filter?.search ||
          [record.name, record.address, record.contactEmail, record.contactPhone].some((value) =>
  ...
  async reserveNextCode(tenantId: string) {
    const bucket = this.getBucket(tenantId);
    bucket.sequence += 1;
    return `CAMP-${String(bucket.sequence).padStart(3, "0")}`;
  }
...
interface CampusDocument extends CampusRecord {
  _id: string;
...
export class MongoCampusRepository implements CampusRepository {
  constructor(
    private readonly collection: CollectionAdapter<CampusDocument>,
    private readonly sequence: CollectionAdapter<SequenceDocument>,
  ) {}
  private from(row: CampusDocument | null) {
    if (!row) return null;
    const { _id: _id, normalizedCode: _code, normalizedName: _name, ...record } = row;
    return clone(record);
  }
  private doc(record: CampusRecord): CampusDocument {
    return {
      _id: record.id,
      ...record,
      normalizedCode: record.code.toLowerCase(),
      normalizedName: record.name.trim().toLowerCase().replace(/\s+/g, " "),
    };
  }
  async list(tenantId: string, filter?: CampusListFilter) {
    ...
    return rows
      .map((row) => this.from(row) as CampusRecord)
      .filter(
        (record) =>
          !filter?.search ||
          [record.name, record.address, record.contactEmail, record.contactPhone].some((value) =>
  ...
  async reserveNextCode(tenantId: string) {
    ...
    const result = await this.sequence.findOneAndUpdate(
      { tenantId: owner, _id: `campus:${owner}` },
      { $inc: { value: 1 }, $setOnInsert: { tenantId: owner } },
      { upsert: true, returnDocument: "after" },
    );
    return `CAMP-${String(result?.value ?? 1).padStart(3, "0")}`;
  }
  async create(tenantId: string, input: CampusCreateInput & { code: string }) {
    const timestamp = now();
    const record: CampusRecord = {
      id: `campus_${crypto.randomUUID()}`,
      tenantId,
      ...input,
      status: "ACTIVE",
      createdAt: timestamp,
...
let defaultRepository: Promise<CampusRepository> | undefined;
export async function createCampusRepository(
  env: MongoEnvLike = runtimeEnv(),
): Promise<CampusRepository> {
  if (!env.MONGODB_URI && !env.MONGODB_URI_DEV && !env.MONGODB_URI_PROD && !env.MONGODB_URI_TEST)
    return new InMemoryCampusRepository();
  const collection = await getCollection<CampusDocument>("settings_campuses", env);
  const sequence = await getCollection<SequenceDocument>("settings_sequences", env);
  ...
  return new MongoCampusRepository(
    createMongoCollectionAdapter(collection),
    createMongoCollectionAdapter(sequence),
  );
}
async function getDefaultRepository() {
  defaultRepository ??= createCampusRepository();
...
export const campusRepository: CampusRepository = {
  ...
  reserveNextCode: async (...a) => (await getDefaultRepository()).reserveNextCode(...a),
  update: async (...a) => (await getDefaultRepository()).update(...a),
  deactivate: async (...a) => (await getDefaultRepository()).deactivate(...a),
  reactivate: async (...a) => (await getDefaultRepository()).reactivate(...a),
};

...
import {
  campusRepository as defaultRepository,
  type CampusRepository,
} from "./campuses.repository";
import {
  validateCampusCreateInput,
  validateCampusListFilter,
  validateCampusUpdateInput,
} from "./campuses.validator";
...
export async function listCampuses(
  ...
  const records = await resolveRepository(deps).list(
    getTenantId(context),
    validateCampusListFilter(filter),
  );
...
export async function createCampus(
  ...
  const duplicate = (await repository.list(tenantId)).some(
    (campus) =>
      campus.name.trim().toLowerCase().replace(/\s+/g, " ") ===
...
export async function updateCampus(
  ...
  if (
    payload.name &&
    (await repository.list(tenantId)).some(
      (campus) =>
        campus.id !== id &&
        campus.name.trim().toLowerCase().replace(/\s+/g, " ") ===
          payload.name!.trim().toLowerCase().replace(/\s+/g, " "),
...
export async function deactivateCampus(
  ...
  if (activeCampuses.length <= 1)
    throw new BadRequestError("the only active campus cannot be deactivated");
  const updated = await repository.deactivate(tenantId, id);
  log(deps, context, `campus.deactivated:${existing.code}`);
  return toCampusView(updated);
}

export async function reactivateCampus(
  context: RequestContext,
  id: string,
  deps?: CampusServiceDeps,
): Promise<CampusView | null> {

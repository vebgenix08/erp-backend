...
const unitKey = (campusId: string, type: string, affiliationId: string) =>
  `${campusId}:${type}:${affiliationId}`.toLowerCase();
const clone = (record: CampusSetupRecord): CampusSetupRecord => ({
  campus: {
    ...record.campus,
    createdAt: new Date(record.campus.createdAt),
    updatedAt: new Date(record.campus.updatedAt),
    deactivatedAt: record.campus.deactivatedAt ? new Date(record.campus.deactivatedAt) : undefined,
  },
  academicUnits: record.academicUnits.map((unit) => ({
    ...unit,
    createdAt: new Date(unit.createdAt),
    updatedAt: new Date(unit.updatedAt),
    deactivatedAt: unit.deactivatedAt ? new Date(unit.deactivatedAt) : undefined,
  })),
});
...
export class InMemoryCampusSetupRepository implements CampusSetupRepository {
  ...
  async create(tenantId: string, input: CampusSetupCreateInput) {
    ...
    if (
      [...this.campuses.values()].some(
        (campus) =>
          campus.tenantId === tenant && normalizedName(campus.name) === normalizedName(input.name),
      )
    )
      throw new ConflictError("campus name must be unique within the tenant");
    const next = (this.sequences.get(tenant) ?? 0) + 1;
    this.sequences.set(tenant, next);
    const timestamp = new Date();
    const campusId = `campus_${crypto.randomUUID()}`;
    const campus: CampusRecord = {
      id: campusId,
      tenantId: tenant,
      code: `CAMP-${String(next).padStart(3, "0")}`,
      name: input.name,
      status: "ACTIVE",
    ...
    const units = input.academicUnits.map((item) => {
      ...
      return {
        ...
        tenantId: tenant,
        campusId,
        code: `UNIT-${id.slice(-6).toUpperCase()}`,
        ...item,
    ...
    for (const unit of units) this.academicUnits.set(unit.id, unit);
...
interface UnitDocument extends CampusAcademicUnitRecord {
  _id: string;
  unitKey: string;
}
...
class MongoCampusSetupRepository implements CampusSetupRepository {
  constructor(
    private readonly campuses: TenantMongoCollection<CampusDocument>,
    private readonly units: TenantMongoCollection<UnitDocument>,
  ...
  async create(tenantId: string, input: CampusSetupCreateInput) {
    ...
    return withTransaction(
      async (session) => this.createInTransaction(tenant, input, session ?? undefined),
      { env: this.env, context: { tenantId: tenant } },
    );
  }
  private async createInTransaction(
    tenantId: string,
    input: CampusSetupCreateInput,
    session?: ClientSession,
  ) {
    const options = session ? { session } : {};
    if (
      await this.campuses.findOne({ tenantId, normalizedName: normalizedName(input.name) }, options)
    )
      throw new ConflictError("campus name must be unique within the tenant");
    const sequence = await this.sequences.findOneAndUpdate(
      { tenantId, _id: `campus:${tenantId}` },
    ...
    const campus: CampusRecord = {
      id: campusId,
      tenantId,
      code: `CAMP-${String(sequence?.value ?? 1).padStart(3, "0")}`,
      name: input.name,
      status: "ACTIVE",
      address: input.address,
      contactEmail: input.contactEmail,
    ...
    await this.campuses.insertOne(
      {
        _id: campus.id,
        ...campus,
        normalizedCode: campus.code.toLowerCase(),
        normalizedName: normalizedName(campus.name),
      },
      options,
    );
    ...
    if (academicUnits.length)
      await this.units.insertMany(
        academicUnits.map((unit) => ({
          _id: unit.id,
          ...unit,
          unitKey: unitKey(unit.campusId, unit.type, unit.curriculumOrAffiliationId),
        })),
        options,
      );
...
const hasMongo = (env: MongoEnvLike) =>
  Boolean(env.MONGODB_URI || env.MONGODB_URI_DEV || env.MONGODB_URI_PROD || env.MONGODB_URI_TEST);
export async function createCampusSetupRepository(
  env: MongoEnvLike = runtimeEnv(),
): Promise<CampusSetupRepository> {
  ...
  return new MongoCampusSetupRepository(
    createTenantMongoCollection(campuses),
    createTenantMongoCollection(units),
    createTenantMongoCollection(db.collection<SequenceDocument>("settings_sequences")),
    env,
  );
}
let singleton: Promise<CampusSetupRepository> | undefined;
export const campusSetupRepository: CampusSetupRepository = {
  create: async (...args) => (await (singleton ??= createCampusSetupRepository())).create(...args),
};

...
export function validateCampusSetupCreateInput(input: unknown): CampusSetupCreateInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("campus setup input is required");
  ...
  const keys = academicUnits.map((unit) =>
    `${unit.type}:${unit.curriculumOrAffiliationId}`.toLowerCase(),
  );
  if (new Set(keys).size !== keys.length)
    throw new BadRequestError("duplicate academic units are not allowed");

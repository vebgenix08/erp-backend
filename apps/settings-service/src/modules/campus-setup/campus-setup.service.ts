...
export interface CampusSetupServiceDeps {
  repository?: CampusSetupRepository;
}
export async function createCampusSetup(
  context: RequestContext,
  input: unknown,
  deps?: CampusSetupServiceDeps,
): Promise<CampusSetupView> {
  requireAuth(context.authContext);
  requirePermission(context.authContext, campusPermissions.create);
  const record = await (deps?.repository ?? campusSetupRepository).create(
    requireTenantId(context.tenantContext),
    validateCampusSetupCreateInput(input),
  );
  return {
    campus: {
      ...record.campus,
      createdAt: record.campus.createdAt.toISOString(),
      updatedAt: record.campus.updatedAt.toISOString(),
      deactivatedAt: record.campus.deactivatedAt?.toISOString(),
    },
    academicUnits: record.academicUnits.map((unit) => ({
      ...unit,
      createdAt: unit.createdAt.toISOString(),
      updatedAt: unit.updatedAt.toISOString(),
      deactivatedAt: unit.deactivatedAt?.toISOString(),
    })),
  };
}

...
const context: RequestContext = {
  requestId: "request_setup",
  path: "graphql:createCampusSetup",
  method: "POST",
  headers: {},
  query: {},
  params: {},
  ...
  authContext: {
    ...
    user: { id: "admin", permissions: ["settings.campuses.create"], source: "request" },
  },
};
test("campus setup atomically creates a campus and all selected academic units", async () => {
  ...
  const result = await createCampusSetup(
    context,
    {
...
test("one campus can own independent CBSE, State Board and ICSE school units", async () => {
  ...
  assert.equal(
    result.academicUnits
      .map((unit) => unit.curriculumOrAffiliationId)
      .sort()
      .join(","),

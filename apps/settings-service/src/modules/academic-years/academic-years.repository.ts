...
export class InMemoryAcademicYearRepository implements AcademicYearRepository {
  ...
  async list(tenantId: string, filter?: AcademicYearListFilter) {
    return [...this.getBucket(tenantId).records.values()]
      ...
      .sort(
        (left, right) =>
          left.startDate.localeCompare(right.startDate) || left.code.localeCompare(right.code),
      )
      .map(clone);
  }
  ...
  async getByCode(tenantId: string, code: string) {
    const record = [...this.getBucket(tenantId).records.values()].find(
  ...
  async activate(tenantId: string, id: string) {
    ...
    for (const [recordId, record] of bucket.records.entries()) {
      bucket.records.set(recordId, {
        ...
        closedAt:
          recordId === id ? undefined : record.status === "ACTIVE" ? nowValue : record.closedAt,
        updatedAt: nowValue,
      });
    }
  ...
  async close(tenantId: string, id: string, reason: string) {
    ...
    if (!existing) return null;
    return this.updateRecord(tenantId, id, {
      ...existing,
      status: "CLOSED",
      closedAt: now(),
  ...
  async reopen(tenantId: string, id: string, reason: string) {
    const existing = await this.getById(tenantId, id);
    if (!existing) return null;
    return this.updateRecord(tenantId, id, {
      ...existing,
  ...
  private async updateRecord(tenantId: string, id: string, record: AcademicYearRecord) {
    this.getBucket(tenantId).records.set(id, record);
    return clone(record);
  }
}

interface AcademicYearDocument extends Omit<AcademicYearRecord, "status"> {
  _id: string;
...
export class MongoAcademicYearRepository implements AcademicYearRepository {
  ...
  private from(row: AcademicYearDocument | null) {
    if (!row) return null;
    const { _id: _id, normalizedCode: _code, ...record } = row;
    return clone({ ...record, status: record.status === "INACTIVE" ? "DRAFT" : record.status });
  }
  private doc(record: AcademicYearRecord): AcademicYearDocument {
    return { _id: record.id, ...record, normalizedCode: record.code.toLowerCase() };
  }
  async list(tenantId: string, filter?: AcademicYearListFilter) {
    const rows = await this.collection.findMany(
      { tenantId, ...(filter?.status ? { status: filter.status } : {}) },
      { sort: { startDate: 1, code: 1 } },
    );
    return rows.map((row) => this.from(row) as AcademicYearRecord);
  }
  async getById(tenantId: string, id: string) {
    return this.from(await this.collection.findOne({ _id: id, tenantId }));
  }
  async getByCode(tenantId: string, code: string) {
    return this.from(
      await this.collection.findOne({ tenantId, normalizedCode: code.toLowerCase() }),
    );
  }
  async create(tenantId: string, input: AcademicYearCreateInput) {
    const timestamp = now();
    const record: AcademicYearRecord = {
      id: `academic_year_${crypto.randomUUID()}`,
      tenantId,
      ...input,
      status: "DRAFT",
      createdAt: timestamp,
    ...
    await this.collection.insertOne(this.doc(record));
    return clone(record);
  }
  async update(tenantId: string, id: string, input: AcademicYearUpdateInput) {
    const existing = await this.getById(tenantId, id);
    ...
    await this.collection.replaceOne({ _id: id, tenantId }, this.doc(updated));
    return clone(updated);
  }
  async activate(tenantId: string, id: string) {
    const existing = await this.getById(tenantId, id);
    ...
    for (const row of rows) {
      const updated: AcademicYearRecord = {
        ...row,
        status: row.id === id ? "ACTIVE" : row.status === "ACTIVE" ? "CLOSED" : row.status,
        activatedAt: row.id === id ? timestamp : row.activatedAt,
        closedAt: row.id === id ? undefined : row.status === "ACTIVE" ? timestamp : row.closedAt,
        updatedAt: timestamp,
      };
      await this.collection.replaceOne({ _id: row.id, tenantId }, this.doc(updated));
    }
...
let defaultRepository: Promise<AcademicYearRepository> | undefined;
export async function createAcademicYearRepository(
  env: MongoEnvLike = runtimeEnv(),
): Promise<AcademicYearRepository> {
  if (!env.MONGODB_URI && !env.MONGODB_URI_DEV && !env.MONGODB_URI_PROD && !env.MONGODB_URI_TEST)
    return new InMemoryAcademicYearRepository();
  const collection = await getCollection<AcademicYearDocument>("settings_academic_years", env);
  await collection.createIndex({ tenantId: 1, normalizedCode: 1 }, { unique: true });
  await collection.createIndex(
    { tenantId: 1, status: 1 },
    { unique: true, partialFilterExpression: { status: "ACTIVE" } },
  );
  return new MongoAcademicYearRepository(createMongoCollectionAdapter(collection));
}
async function getDefaultRepository() {
  defaultRepository ??= createAcademicYearRepository();
  return defaultRepository;
}
export const academicYearRepository: AcademicYearRepository = {
  list: async (...a) => (await getDefaultRepository()).list(...a),
  getById: async (...a) => (await getDefaultRepository()).getById(...a),
  getByCode: async (...a) => (await getDefaultRepository()).getByCode(...a),
  create: async (...a) => (await getDefaultRepository()).create(...a),
  update: async (...a) => (await getDefaultRepository()).update(...a),
  activate: async (...a) => (await getDefaultRepository()).activate(...a),
  close: async (...a) => (await getDefaultRepository()).close(...a),
  reopen: async (...a) => (await getDefaultRepository()).reopen(...a),
};

...
export function registerAcademicYearRoutes(
  ...
  router.route("GET", "/academic-years", async (context) => {
    const result = await listAcademicYearsUseCase(
      toSettingsContext(context),
      deps,
      validateAcademicYearListFilter(context.query),
    );
  ...
  router.route("GET", "/academic-years/:id", async (context) => {
    const result = await getAcademicYearUseCase(
      toSettingsContext(context),
      context.params.id ?? "",
      deps,
    );
  ...
  router.route("PATCH", "/academic-years/:id", async (context) => {
    const result = await updateAcademicYearUseCase(
      toSettingsContext(context),
      context.params.id ?? "",
      context.body,
      deps,
  ...
  router.route("POST", "/academic-years/:id/activate", async (context) => {
    const result = await activateAcademicYearUseCase(
      toSettingsContext(context),
      context.params.id ?? "",
      deps,
    );

...
import {
  academicYearRepository as defaultRepository,
  type AcademicYearRepository,
} from "./academic-years.repository";
import { toAcademicYearView } from "./academic-years.mapper";
import {
  validateAcademicYearCreateInput,
  validateAcademicYearListFilter,
  validateAcademicYearUpdateInput,
...
function log(
  deps: AcademicYearServiceDeps | undefined,
...
export async function listAcademicYears(
  ...
  const records = await resolveRepository(deps).list(
    getTenantId(context),
    validateAcademicYearListFilter(filter),
  );
...
export async function createAcademicYear(
  ...
  const initialized =
    existingYears.length === 1 ? await repository.activate(tenantId, created.id) : created;
...
export async function updateAcademicYear(
  ...
  if (existing.status === "CLOSED")
    throw new BadRequestError("closed academic years cannot be edited; reopen it first");
...
export async function activateAcademicYear(
  ...
  if (!existing) return null;
  if (existing.status === "CLOSED")
    throw new BadRequestError("closed academic year must be reopened before activation");
...
function requireReason(input: unknown): string {
  ...
  if (reason.length < 5)
    throw new BadRequestError("a lifecycle reason of at least 5 characters is required");
  return reason;
}
export async function closeAcademicYear(
  ...
  ensure(context, academicYearPermissions.close);
  const repository = resolveRepository(deps);
  const tenantId = getTenantId(context);
  const existing = await repository.getById(tenantId, id);
  if (!existing) return null;
  if (existing.status !== "ACTIVE")
    throw new BadRequestError("only the active academic year can be closed");
  const updated = await repository.close(tenantId, id, requireReason(reason));
  log(deps, context, `academic-year.closed:${existing.code}`);
  return toAcademicYearView(updated);
}
export async function reopenAcademicYear(
  ...
  ensure(context, academicYearPermissions.reopen);
  const repository = resolveRepository(deps);
  const tenantId = getTenantId(context);
  const existing = await repository.getById(tenantId, id);
  if (!existing) return null;
  if (existing.status !== "CLOSED")
    throw new BadRequestError("only a closed academic year can be reopened");
  const updated = await repository.reopen(tenantId, id, requireReason(reason));
  log(deps, context, `academic-year.reopened:${existing.code}`);
  return toAcademicYearView(updated);
}

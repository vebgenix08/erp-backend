...
export function registerFeatureFlagRoutes(
  ...
  router.route("GET", "/feature-flags", async (context: RequestContext) =>
    jsonResponse(200, await listFeatureFlags(context, deps)),
  );
  ...
  router.route("POST", "/feature-flags", async (context: RequestContext) =>
    jsonResponse(201, await createFeatureFlag(context.body, context, deps)),
  );

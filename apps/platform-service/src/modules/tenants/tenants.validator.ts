...
function validateMonth(value: unknown): number | undefined {
  ...
  if (!Number.isInteger(month) || month < 1 || month > 12) {
    throw new ValidationError([
      { field: "academicYearStartMonth", message: "must be an integer between 1 and 12" },
    ]);
  }
  return month;
}
...
export function validateTenantCreateInput(input: Record<string, unknown>): TenantCreateInput {
  ...
  const contactPhone =
    input.contactPhone === undefined
      ? undefined
      : validatePhone(input.contactPhone, "contactPhone");
  ...
  if (!clientRequestId.success)
    errors.push({
      ...
      message: clientRequestId.errors[0] ?? "clientRequestId is required",
    });
  if (!isTenantType(type)) errors.push({ field: "type", message: "type is required" });
  if (contactEmail !== undefined && !contactEmail.success)
    errors.push({
      field: "contactEmail",
      message: contactEmail.errors[0] ?? "contactEmail is invalid",
    });
  if (contactPhone !== undefined && !contactPhone.success)
    errors.push({
      field: "contactPhone",
      message: contactPhone.errors[0] ?? "contactPhone is invalid",
    });
  ...
  if (input.contactEmail !== undefined) {
    if (
      ...
      if (!value.success)
        errors.push({
          field: "contactEmail",
          message: value.errors[0] ?? "contactEmail is invalid",
        });
  ...
  if (input.contactPhone !== undefined) {
    if (
      ...
      if (!value.success)
        errors.push({
          field: "contactPhone",
          message: value.errors[0] ?? "contactPhone is invalid",
        });

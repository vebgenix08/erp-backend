...
test("update tenant changes mutable fields", async () => {
  ...
  const created = await createTenantUseCase(
    createTenantFixture({ name: "Old Name", code: "OLD" }),
    createPlatformAdminContext(),
    { repository },
  );

  const updated = await updateTenantUseCase(
    String(created?.id ?? ""),
    { name: "New Name" },
    createPlatformAdminContext(),
    { repository },
...
test("update tenant rejects duplicate code", async () => {
  const repository = new InMemoryTenantRepository();
  const first = await createTenantUseCase(
    createTenantFixture({ name: "First", code: "FIRST" }),
    createPlatformAdminContext(),
    { repository },
  );
  await createTenantUseCase(
    createTenantFixture({ name: "Second", code: "SECOND", type: "COLLEGE" }),
    createPlatformAdminContext(),
    { repository },
  );

...
test("deactivate tenant marks the tenant inactive with a deactivated timestamp", async () => {
  ...
  const created = await createTenantUseCase(createTenantFixture({ name: "Dormant School", code: "DORMANT" }), { repository });

  const deactivated = await deactivateTenantUseCase(String(created?.id ?? ""), { repository });

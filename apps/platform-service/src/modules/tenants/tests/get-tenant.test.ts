...
test("get tenant returns the stored tenant view", async () => {
  ...
  const created = await createTenantUseCase(createTenantFixture({ name: "City College", code: "CITY", type: "COLLEGE" }), { repository });

  const found = await getTenantUseCase(String(created?.id ?? ""), { repository });

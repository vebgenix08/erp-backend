...
import type { TenantEntitlementInput, TenantEntitlementStatus } from "./entitlements.model";
import { isTenantCapabilityCode } from "../capability-catalog/capability-catalog.service";
export function validateTenantEntitlementInput(input: unknown): TenantEntitlementInput {
  ...
  const featureCode =
    typeof value.featureCode === "string" ? value.featureCode.trim().toUpperCase() : "";
  const status =
  ...
  if (status !== "ENABLED" && status !== "DISABLED") throw new BadRequestError("status is invalid");
  const limits =
    value.limits && typeof value.limits === "object" && !Array.isArray(value.limits)
      ? (value.limits as Record<string, number>)
      : undefined;
  if (limits && Object.values(limits).some((limit) => !Number.isInteger(limit) || limit < 0))
    throw new BadRequestError("entitlement limits must be non-negative integers");

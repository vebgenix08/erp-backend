...
export function validatePlatformIntegrationInput(input: unknown): PlatformIntegrationInput {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new BadRequestError("integration input is required");
  const v = input as Record<string, unknown>;
  const code = String(v.code ?? "").toUpperCase();
  const status = String(v.status ?? "").toUpperCase();
  if (!CODES.includes(code)) throw new BadRequestError("integration code is invalid");
  if (!STATUSES.includes(status)) throw new BadRequestError("integration status is invalid");
  const secretReference =
    typeof v.secretReference === "string" ? v.secretReference.trim() : undefined;
  if (secretReference && secretReference.includes("arn:aws:secretsmanager") === false)

...
import { listPlatformIntegrations, setPlatformIntegration } from "../integrations.service";
const context: RequestContext = {
  ...
  authContext: {
    ...
    user: {
      id: "admin",
      role: "SUPER_ADMIN",
      permissions: ["platform.integrations.read", "platform.integrations.manage"],
      source: "request",
    },
  },
};
test("integration configuration stores a secret reference and never a secret value", async () => {
  ...
  await setPlatformIntegration(
    {
      code: "EMAIL",
      status: "CONFIGURED",
      secretReference: "arn:aws:secretsmanager:ap-south-1:123456789012:secret:email",
  ...
  assert.ok((records[0]?.secretReference ?? "").startsWith("arn:aws:secretsmanager:"));
});

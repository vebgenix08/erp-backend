...
export function createPlatformGraphqlContext(event: PlatformGraphqlEvent): RequestContext {
  ...
  const groupList = Array.isArray(groups)
    ...
    : typeof groups === "string"
      ...
      : [];
  const role =
    stringClaim(claims, "custom:role", "role") ??
    (groupList.includes("SUPER_ADMIN") ? "SUPER_ADMIN" : undefined);
...
async function executePlatformGraphql(
  event: PlatformGraphqlEvent,
  deps: PlatformGraphqlDeps = {},
): Promise<unknown> {
  ...
  switch (event.info.fieldName) {
    case "currentTenantSummary": {
      ...
      const repository = await (deps.repository ??
        (await import("../modules/tenants/tenants.repository")).createTenantRepository());
    ...
    case "tenants": {
      ...
      const first =
        typeof args.first === "number" ? Math.min(Math.max(Math.trunc(args.first), 1), 100) : 25;
      const parsedCursor =
        typeof args.after === "string" && args.after.startsWith("cursor:")
      ...
      const page = result.items.map((tenant) => ({
        ...tenant,
        ...
        },
        {
          ...
          limit:
            typeof args.first === "number"
              ? Math.min(Math.max(Math.trunc(args.first), 1), 100)
              : 50,
          offset:
            typeof args.after === "string" && args.after.startsWith("cursor:")
              ? Math.max(0, Number.parseInt(args.after.slice(7), 10) + 1)
              : 0,
        },
      );
    ...
    case "provisionTenant":
      return provisionTenant(input(args), context, {
        ...((deps.provisioning?.tenants ?? deps.repository)
          ? { tenants: deps.provisioning?.tenants ?? deps.repository }
          : {}),
        bootstrap: {
          ...deps.provisioning?.bootstrap,
          invitePort:
            deps.provisioning?.bootstrap?.invitePort ?? createCognitoFirstAdminInvitePort(),
        },
        ...((deps.provisioning?.auditLogs ?? deps.auditLogs)
          ? { auditLogs: deps.provisioning?.auditLogs ?? deps.auditLogs }
          : {}),
      });
    case "updateTenant": {
      const payload = input(args);
    ...
    case "requestTenantDeletion": {
      const result = await requestTenantDeletionUseCase(
        requiredString(args, "tenantId"),
    ...
    case "confirmTenantDeletion": {
      const result = await confirmTenantDeletionUseCase(
        requiredString(args, "tenantId"),
        context,
        deps,
      );
    ...
    case "resendFirstAdminBootstrapInvite": {
      const tenantId = requiredString(args, "tenantId");
      const result = await resendFirstAdminBootstrapInvite(tenantId, context, {
        ...deps.bootstrap,
        invitePort: deps.bootstrap?.invitePort ?? createCognitoFirstAdminInvitePort(),
      });
      await appendAuditLog(
        {
          ...
          tenantId,
          action: firstAdminInviteAuditAction(result.status),
          entityType: "FIRST_ADMIN_BOOTSTRAP",
          entityId: result.id,
          details: { status: result.status, email: result.adminEmail },
        },
        deps.auditLogs,
      );
      return result;
    ...
    case "completeFirstAdminBootstrap": {
      const result = await completeFirstAdminBootstrap(
        requiredString(args, "tenantId"),
...
const AUDITED_MUTATIONS: Record<string, { action: string; entityType: string }> = {
  ...
  createFirstAdminBootstrap: {
    action: "TENANT_ADMIN_BOOTSTRAP_CREATED",
    entityType: "FIRST_ADMIN_BOOTSTRAP",
  },
  completeFirstAdminBootstrap: {
    action: "TENANT_ADMIN_BOOTSTRAP_COMPLETED",
    entityType: "FIRST_ADMIN_BOOTSTRAP",
  },
  setTenantEntitlement: { action: "TENANT_ENTITLEMENT_CHANGED", entityType: "TENANT_ENTITLEMENT" },
  setPlatformIntegration: {
    action: "PLATFORM_INTEGRATION_CHANGED",
    entityType: "PLATFORM_INTEGRATION",
  },
};

export async function handlePlatformGraphql(
  event: PlatformGraphqlEvent,
  deps: PlatformGraphqlDeps = {},
): Promise<unknown> {
  ...
  await appendAuditLog(
    {
      ...
      tenantId:
        typeof record.tenantId === "string"
          ? record.tenantId
          : typeof record.id === "string" && audit.entityType === "TENANT"
            ? record.id
            : undefined,
      action: audit.action,

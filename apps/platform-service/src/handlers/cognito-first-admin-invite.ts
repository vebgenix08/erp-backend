...
interface InviteEnvironment {
  COGNITO_USER_POOL_ID?: string;
  DEPLOY_REGION?: string;
}

export async function assertSesRecipientEligible(
  client: SESv2Client,
  email: string,
): Promise<void> {
  ...
  try {
    ...
    verified =
      identity.VerifiedForSendingStatus === true && identity.VerificationStatus === "SUCCESS";
  } catch {
    verified = false;
  }
  if (!verified) {
...
export function createCognitoFirstAdminInvitePort(
  env: InviteEnvironment = runtimeEnvironment(),
): FirstAdminInvitePort {
  ...
  const client = new CognitoIdentityProviderClient(
    env.DEPLOY_REGION ? { region: env.DEPLOY_REGION } : {},
  );
  ...
  return {
    async sendFirstAdminInvite(input) {
      ...
      try {
        ...
        await client
          .send(new AdminDeleteUserCommand({ UserPoolId: userPoolId, Username: input.adminEmail }))
          .catch(() => undefined);
        throw error;
      }

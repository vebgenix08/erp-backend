...
export interface InviteEmailProviderResult {
  messageId: string;
}
export interface InviteEmailProvider {
  send(input: InviteEmailCreateInput): Promise<InviteEmailProviderResult>;
}
    ...
    const response = await this.client.send(
      new SendEmailCommand({
        ...
        EmailTags: [
          ...
          { Name: "inviteId", Value: input.inviteId },
        ],
      }),
    );
...
export function createRuntimeInviteEmailProvider(

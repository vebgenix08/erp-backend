import type { InviteEmailListFilter, InviteEmailRecord } from "./invite-email.model";
...
export interface InviteEmailRepository {
  ...
  update(
    tenantId: string,
    id: string,
    input: Partial<InviteEmailRecord>,
  ): Promise<InviteEmailRecord>;
  updateByMessageId(
    messageId: string,
    input: Partial<InviteEmailRecord>,
  ): Promise<InviteEmailRecord | null>;
}
...
interface InviteEmailDocument extends InviteEmailRecord {
  _id: string;
}

export class MongoInviteEmailRepository implements InviteEmailRepository {
  constructor(
    private readonly collection: import("@school-erp/mongodb").CollectionAdapter<InviteEmailDocument>,
    private readonly providerCollection: import("@school-erp/mongodb").PlatformCollectionAdapter<InviteEmailDocument>,
  ) {}
  async list(tenantId: string, filter: InviteEmailListFilter = {}) {
    const query: Record<string, unknown> = { tenantId };
    if (filter.status) query.status = filter.status;
    const records = await this.collection.findMany(query as never, {
      sort: { createdAt: -1 },
      limit: 200,
    });
    return records
      .map(({ _id: _ignored, ...record }) => record)
      .filter((record) => !filter.search || matchesSearch(record, filter.search));
  }
  ...
  async updateByMessageId(messageId: string, input: Partial<InviteEmailRecord>) {
    ...
    const document = await this.providerCollection.findOneAndUpdate(
...
export async function createInviteEmailRepository(): Promise<InviteEmailRepository> {
  const { createMongoCollectionAdapter, createPlatformMongoCollectionAdapter, getCollection } =
    await import("@school-erp/mongodb");
  ...
  return new MongoInviteEmailRepository(
    createMongoCollectionAdapter(collection),
    createPlatformMongoCollectionAdapter(collection),
  );
}

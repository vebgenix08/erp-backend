...
export function loadInfrastructureConfig(
  overrides: InfrastructureConfigOverrides = {},
): EnvironmentConfig {
  ...
  const inputs: EnvironmentInputs = {
    ...
    accountId,
    region,
    ...(process.env.S3_BUCKET_PREFIX?.trim()
      ? { bucketPrefix: process.env.S3_BUCKET_PREFIX.trim() }
      : {}),
    ...(process.env.GITHUB_REPOSITORIES?.trim()
      ? {
          githubRepositories: process.env.GITHUB_REPOSITORIES.split(",")
    ...
    ...(process.env.GITHUB_ENVIRONMENTS?.trim()
      ? {
          githubEnvironments: process.env.GITHUB_ENVIRONMENTS.split(",")
            .map((value) => value.trim())
            .filter(Boolean),
        }
      : {}),
    ...(process.env.COGNITO_DOMAIN_PREFIX?.trim()
      ? { cognitoDomainPrefix: process.env.COGNITO_DOMAIN_PREFIX.trim() }
      : {}),
  };

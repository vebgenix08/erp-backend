...
export class LambdaServicesStack extends Stack {
  ...
  public readonly numberingFunction: lambda.Function;
  ...
  constructor(scope: Construct, id: string, props: LambdaServicesStackProps) {
    ...
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "StorageFilesMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.storageOperationalFunction);
    this.storageOperationalFunction.addToRolePolicy(
    ...
    this.numberingFunction = new NodejsFunction(this, "SettingsNumberingFunction", {
      functionName: `settings-numbering-${props.config.environment}`,
      description: "IAM-only tenant numbering allocation owned by settings-service",
      runtime: lambda.Runtime.NODEJS_20_X,
      entry: path.resolve(
        process.cwd(),
        "..",
    ...
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "SettingsNumberingMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.numberingFunction);
    new logs.LogRetention(this, "SettingsNumberingLogRetention", {
      logGroupName: `/aws/lambda/${this.numberingFunction.functionName}`,
      retention: props.config.logRetentionDays,
    });
    this.addFunctionAlarms("SettingsNumbering", this.numberingFunction, props.config);
    ...
    this.admissionsGraphqlFunction = new NodejsFunction(this, "AdmissionsGraphqlFunction", {
      ...
      timeout: props.config.lambdaTimeout,
      bundling: {
        externalModules: ["@aws-sdk/client-secrets-manager", "@aws-sdk/client-eventbridge"],
      },
      environment: {
        environment: props.config.environment,
    ...
    this.commsGraphqlFunction = new NodejsFunction(this, "CommsGraphqlFunction", {
      ...
      environment: {
        ...
        MONGODB_SECRET_NAME: props.secretNames.mongodb,
        SES_FROM_EMAIL: props.config.sesFromEmail,
        SES_CONFIGURATION_SET_NAME: props.config.sesConfigurationSetName,
      },
    });
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "CommsGraphqlMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.commsGraphqlFunction);
    this.commsGraphqlFunction.addToRolePolicy(
      new iam.PolicyStatement({ actions: ["ses:SendEmail"], resources: ["*"] }),
    );
    new logs.LogRetention(this, "CommsGraphqlLogRetention", {
      logGroupName: `/aws/lambda/${this.commsGraphqlFunction.functionName}`,
      retention: props.config.logRetentionDays,
    });
    ...
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "MongoDbSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.platformGraphqlFunction);
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "SettingsGraphqlMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.settingsGraphqlFunction);
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "IdentityGraphqlMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.identityGraphqlFunction);
    this.identityGraphqlFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
        ...
        resources: [
          `arn:aws:cognito-idp:${props.config.region}:${props.config.accountId}:userpool/${props.cognitoUserPoolId}`,
        ],
      }),
    );
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "AcademicsGraphqlMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.academicsGraphqlFunction);
    this.academicsGraphqlFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["events:PutEvents"],
        resources: [
          `arn:aws:events:${props.config.region}:${props.config.accountId}:event-bus/${props.eventBusName}`,
        ],
      }),
    );
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "AdmissionsGraphqlMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.admissionsGraphqlFunction);
    this.admissionsGraphqlFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["events:PutEvents"],
        resources: [
          `arn:aws:events:${props.config.region}:${props.config.accountId}:event-bus/${props.eventBusName}`,
    ...
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "FinanceGraphqlMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.financeGraphqlFunction);
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "FinanceReceiptMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.financeReceiptFunction);
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "AcademicsDocumentMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(this.academicsDocumentFunction);
    this.financeGraphqlFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["s3:GetObject"],
        resources: [`arn:aws:s3:::${props.documentsBucketName}/*`],
      }),
    );
    this.financeReceiptFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["s3:GetObject"],
        resources: [`arn:aws:s3:::${props.documentsBucketName}/*`],
      }),
    );
    this.academicsDocumentFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["s3:GetObject"],
        resources: [`arn:aws:s3:::${props.documentsBucketName}/*`],
      }),
    );
    this.platformGraphqlFunction.addToRolePolicy(
      new iam.PolicyStatement({
    ...
    emailEventsFunction.addEventSource(
      new eventSources.SqsEventSource(props.emailEventsQueue, {
        batchSize: 10,
    ...
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "EmailEventsMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(emailEventsFunction);
    ...
    new logs.LogRetention(this, "AdmissionsGraphqlLogRetention", {
      logGroupName: `/aws/lambda/${this.admissionsGraphqlFunction.functionName}`,
      retention: props.config.logRetentionDays,
    });
    new logs.LogRetention(this, "FinanceGraphqlLogRetention", {
      logGroupName: `/aws/lambda/${this.financeGraphqlFunction.functionName}`,
      retention: props.config.logRetentionDays,
    });
    new logs.LogRetention(this, "FinanceReceiptLogRetention", {
      logGroupName: `/aws/lambda/${this.financeReceiptFunction.functionName}`,
      retention: props.config.logRetentionDays,
    });
    new logs.LogRetention(this, "AcademicsDocumentLogRetention", {
      logGroupName: `/aws/lambda/${this.academicsDocumentFunction.functionName}`,
      retention: props.config.logRetentionDays,
    });
    ...
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "AdmissionConfirmedMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(admissionConfirmedFunction);
    admissionConfirmedFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["events:PutEvents"],
        resources: [
          `arn:aws:events:${props.config.region}:${props.config.accountId}:event-bus/${props.eventBusName}`,
    ...
    if (props.config.environment === "prod") {
      (
        admissionConfirmedFunction.node.defaultChild as lambda.CfnFunction
      ).reservedConcurrentExecutions = props.config.workerReservedConcurrency;
    }
    ...
    const admissionConfirmedDeadLetterQueue = new sqs.Queue(
      this,
      "AdmissionConfirmedDeadLetterQueue",
      {
    ...
    new events.Rule(this, "AdmissionConfirmedEventRule", {
      ...
      targets: [
        new eventTargets.LambdaFunction(admissionConfirmedFunction, {
          retryAttempts: 2,
          deadLetterQueue: admissionConfirmedDeadLetterQueue,
        }),
      ],
    });
    ...
    const studentEnrolledFunction = new NodejsFunction(this, "StudentEnrolledFunction", {
      ...
      description:
        "Generates the authoritative fee order from an enrolled student and active fee mapping",
      runtime: lambda.Runtime.NODEJS_20_X,
      entry: path.resolve(
        process.cwd(),
        "..",
    ...
    secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "StudentEnrolledMongoSecretReference",
      props.mongodbSecretArn,
    ).grantRead(studentEnrolledFunction);
    ...
    if (props.config.environment === "prod") {
      (
        studentEnrolledFunction.node.defaultChild as lambda.CfnFunction
      ).reservedConcurrentExecutions = props.config.workerReservedConcurrency;
    }
    ...
    new events.Rule(this, "StudentEnrolledEventRule", {
      ...
      targets: [
        new eventTargets.LambdaFunction(studentEnrolledFunction, {
          retryAttempts: 2,
          deadLetterQueue: studentEnrolledDeadLetterQueue,
        }),
      ],
    });
    ...
    const mongoReference = secretsmanager.Secret.fromSecretCompleteArn(
      this,
      "CampusTransferMongoSecretReference",
      props.mongodbSecretArn,
    );
    mongoReference.grantRead(academicsCampusTransferTask);
    mongoReference.grantRead(financeCampusTransferTask);
    for (const [id, fn] of [
      ["AcademicsCampusTransfer", academicsCampusTransferTask],
      ["FinanceCampusTransfer", financeCampusTransferTask],
    ] as const) {
    ...
    const assessAndApplyFinance = new tasks.LambdaInvoke(
      this,
      "AssessAndApplyCampusTransferFinance",
      {
        lambdaFunction: financeCampusTransferTask,
        payloadResponseOnly: true,
        resultPath: "$.finance",
        payload: sfn.TaskInput.fromObject({
          operation: "ASSESS_AND_APPLY",
          "id.$": "$.transfer.id",
          "tenantId.$": "$.transfer.tenantId",
          "studentId.$": "$.transfer.studentId",
          "studentName.$": "$.transfer.studentName",
          "admissionApplicationId.$": "$.transfer.admissionApplicationId",
          "registrationNumber.$": "$.transfer.targetRegistrationNumber",
          "source.$": "$.transfer.source",
          "target.$": "$.transfer.target",
          "requestedBy.$": "$.transfer.requestedBy",
          "financeApproved.$": "$.transfer.financeApproved",
        }),
      },
    );
    const commitAcademicTransfer = new tasks.LambdaInvoke(this, "CommitCampusTransferEnrollment", {
      ...
      payload: sfn.TaskInput.fromObject({
        ...
        "financeAssessment.$": "$.finance.assessment",
      }),
    });
    ...
    const markTransferFailed = new tasks.LambdaInvoke(this, "MarkCampusTransferFailed", {
      ...
      payload: sfn.TaskInput.fromObject({
        ...
        "tenantId.$": "$.transfer.tenantId",
        "transferId.$": "$.transfer.id",
        failureReason: "Campus transfer failed after finance processing; compensation was applied.",
      }),
    });
    const markTransferContextFailed = new tasks.LambdaInvoke(
      this,
      "MarkCampusTransferContextFailed",
      {
        lambdaFunction: academicsCampusTransferTask,
        payloadResponseOnly: true,
        payload: sfn.TaskInput.fromObject({
          ...
          "tenantId.$": "$.tenantId",
          "transferId.$": "$.transferId",
          failureReason: "Campus transfer context could not be loaded.",
        }),
      },
    );
    ...
    const transferDefinition = getTransferContext
      .next(assessAndApplyFinance)
      .next(
        new sfn.Choice(this, "CampusTransferNeedsFinanceReview")
    ...
    const campusTransferStateMachine = new sfn.StateMachine(this, "CampusTransferStateMachine", {
      ...
      logs: {
        destination: new logs.LogGroup(this, "CampusTransferWorkflowLogs", {
          logGroupName: `/aws/vendedlogs/states/campus-transfer-${props.config.environment}`,
          retention: props.config.logRetentionDays,
        }),
    ...
    this.academicsGraphqlFunction.addEnvironment(
      "CAMPUS_TRANSFER_STATE_MACHINE_ARN",
      campusTransferStateMachine.stateMachineArn,
    );
    campusTransferStateMachine.grantStartExecution(this.academicsGraphqlFunction);
    new CfnOutput(this, "CampusTransferStateMachineArn", {
      value: campusTransferStateMachine.stateMachineArn,
    });

    const functionName = (service: string) => `${service}-${props.config.environment}`;
    const addInternalTarget = (
      caller: lambda.Function,
      environmentKey: string,
      targetFunctionName: string,
    ) => {
      const targetArn = Stack.of(this).formatArn({
        service: "lambda",
        resource: "function",
        resourceName: targetFunctionName,
        arnFormat: ArnFormat.COLON_RESOURCE_NAME,
      });
      caller.addEnvironment(environmentKey, targetFunctionName);
      caller.addToRolePolicy(
        new iam.PolicyStatement({
          actions: ["lambda:InvokeFunction"],
          resources: [targetArn, `${targetArn}:*`],
        }),
      );
    };

    const numberingFunctionName = functionName("settings-numbering");
    const numberingConsumers = [
      this.identityGraphqlFunction,
      this.admissionsGraphqlFunction,
      this.academicsGraphqlFunction,
      this.financeGraphqlFunction,
      this.academicsDocumentFunction,
      admissionConfirmedFunction,
      studentEnrolledFunction,
      academicsCampusTransferTask,
      financeCampusTransferTask,
    ];
    for (const consumer of numberingConsumers) {
      addInternalTarget(consumer, "NUMBERING_FUNCTION_NAME", numberingFunctionName);
    }
    addInternalTarget(
      this.identityGraphqlFunction,
      "COMMS_FUNCTION_NAME",
      functionName("comms-service-graphql"),
    );
    ...
    for (const caller of [
      ...
      addInternalTarget(caller, "IDENTITY_FUNCTION_NAME", functionName("identity-service-graphql"));
    }

    for (const [key, targetName] of [
      ["ACADEMICS_FUNCTION_NAME", functionName("academics-service-graphql")],
      ["ADMISSIONS_FUNCTION_NAME", functionName("admissions-service-graphql")],
      ["IDENTITY_FUNCTION_NAME", functionName("identity-service-graphql")],
      ["FINANCE_FUNCTION_NAME", functionName("finance-service-graphql")],
    ] as const) {
      addInternalTarget(this.settingsGraphqlFunction, key, targetName);
    }
    addInternalTarget(
      this.admissionsGraphqlFunction,
      "SETTINGS_FUNCTION_NAME",
      functionName("settings-service-graphql"),
    );
    for (const caller of [this.academicsGraphqlFunction, this.academicsDocumentFunction]) {
      addInternalTarget(caller, "SETTINGS_FUNCTION_NAME", functionName("settings-service-graphql"));
      addInternalTarget(caller, "STORAGE_FUNCTION_NAME", functionName("storage-service-files"));
    }
    for (const caller of [this.financeGraphqlFunction, this.financeReceiptFunction]) {
      addInternalTarget(caller, "SETTINGS_FUNCTION_NAME", functionName("settings-service-graphql"));
      addInternalTarget(
        caller,
        "ACADEMICS_FUNCTION_NAME",
        functionName("academics-service-graphql"),
      );
      addInternalTarget(caller, "STORAGE_FUNCTION_NAME", functionName("storage-service-files"));
    }
    new CfnOutput(this, "SettingsNumberingFunctionName", {
      value: this.numberingFunction.functionName,
    });
  ...
  private addFunctionAlarms(id: string, target: lambda.Function, config: EnvironmentConfig): void {

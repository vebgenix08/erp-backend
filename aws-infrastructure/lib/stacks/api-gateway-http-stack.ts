...
export interface ApiGatewayHttpStackProps extends StackProps {
  ...
  identitySessionFunction: lambda.IFunction;
...
export class ApiGatewayHttpStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: ApiGatewayHttpStackProps) {
    ...
    const storageIntegration = new HttpLambdaIntegration(
      "StorageFilesIntegration",
      props.storageFunction,
    );
    ...
    const identitySessionIntegration = new HttpLambdaIntegration(
      "IdentitySessionIntegration",
      props.identitySessionFunction,
    );
    for (const [path, method] of [
      ["/session/me", apigatewayv2.HttpMethod.GET],
      ["/session/select-tenant", apigatewayv2.HttpMethod.POST],
      ["/session/logout", apigatewayv2.HttpMethod.POST],
    ] as const) {
      this.api.addRoutes({
        path,
        methods: [method],
        integration: identitySessionIntegration,
        authorizer,
      });
    }
    ...
    this.api.addRoutes({
      ...
      integration: new HttpLambdaIntegration(
        "FinanceReceiptPdfIntegration",
        props.financeReceiptFunction,
      ),
      authorizer,
    });
    this.api.addRoutes({
      ...
      methods: [apigatewayv2.HttpMethod.GET],
      integration: new HttpLambdaIntegration(
        "AcademicsStudentDocumentPdfIntegration",
        props.academicsDocumentFunction,
      ),
      authorizer,
    });
